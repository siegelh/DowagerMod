...
- Diplomatic espionage escalation:
  ...
  - With enough EP to pay the modified 900-base-EP cost, confirm Fabricate Casus Belli appears in the first menu even when no third civilization is currently eligible; reduce EP below the modified cost and confirm the mission is hidden
  - Open the second popup with no eligible target; confirm known major civilizations are listed with exact rejection reasons, no rejected civilization has a button, the popup states that no civilization is eligible, and only Cancel is actionable
  - Exercise war-plan, current-war, vassal, defensive-pact, legal-war, unmet, land-accessibility, attitude, and power exclusions; confirm attitude reports the current raw value and required `-3` threshold, while power reports the current percentage and required 70%
  - Prepare a third civilization that the territory-owning AI has met, can legally attack by land, dislikes at raw attitude `-3` or lower, and has at least a 70% target/defender power ratio; confirm it becomes a selectable button while rejected civilizations remain explanatory text
...
- Great Person landmarks (Grand Colosseum, Industrial Zone, Naval Foundry, Research Campus, Commercial District, Grand Bazaar, and all Sacred Grove variants):
  ...
  - Map model scale: confirm Industrial Zone, Naval Foundry, Research Campus, and all 8 Sacred Grove variants render at `fScale = 0.65`; confirm Commercial District and Grand Bazaar use active Plot L-System `<Scale>0.50</Scale>` overrides and remain appropriately smaller than city buildings; confirm Jokamachi uses fixed `fScale = 0.75`; check all three calibrations for readability at normal and strategic zoom
  - Jokamachi rotation: place eight well-separated copies and confirm multiple orientations appear from the same exact eight-angle set; inspect the diagonal `45`, `135`, `225`, and `315` degree candidates for pivot drift, road or neighboring-tile overlap, and terrain clipping
