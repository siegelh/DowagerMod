...
class CvPediaMain( CvPediaScreen.CvPediaScreen ):
	...
	def showScreen(self, iCategory):
		...
		screen.addPanel(self.panelName, "", "", false, true,
	...
	def placeBuildings(self):
		...
		buildingList = self.pediaBuildingScreen.getBuildingSortedList(false)
	...
	def placeWonders(self):
		...
		buildingList = self.pediaBuildingScreen.getBuildingSortedList(true)
	...
	def placeCivs(self):
        ...
        def getNumItems( self, numInfos, getInfo, num ): 
                ' returned a number of items from a list of infos' 
                infoList = [(0,0)] * num 
                for i in range( num ): 
	...
	def placeLeaders(self):
		...
		#replaced to hide non-playable leaders

                leaderList = self.getNumItems( gc.getNumLeaderHeadInfos(), gc.getLeaderHeadInfo, 4 )
	...
	def placeConcepts(self):
		...
		for concept in conceptList:
			...
			screen.setText("tableItem" + str(iRow*nColumns + iColumn), "", u"<font=4>" + concept[0] + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, 0, 0, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_PEDIA_DESCRIPTION, CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT, concept[1])
	...
	def link(self, szLink):
		for i in range(gc.getNumConceptInfos()):
			if (gc.getConceptInfo(i).getType() == szLink):
				...
				self.pediaJump(CvScreenEnums.PEDIA_HISTORY, iEntryId, True)
