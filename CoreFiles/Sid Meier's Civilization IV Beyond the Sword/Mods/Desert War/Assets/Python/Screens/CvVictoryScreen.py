...
import ScreenInput
import PyHelpers
import CvDesertWar	# LOQ 2005-09-19
import time
...
dw = CvDesertWar.CvDesertWar()	# LOQ 2005-09-19
...
class CvVictoryScreen:
	...
	def drawTabs(self):
		...
		# LOQ 2005-09-23: remove UN Resolution Screen
#		if self.bVoteTab:			
...
#			xLink += self.DX_LINK
		...
		# LOQ 2005-09-19: remove Space Ship Screen
...
#			xLink += self.DX_LINK
	...
	def showVictoryConditionScreen(self):
		...
		# LOQ 2005-09-19: overwrite victory screen (original commented out)
		dw.victoryScreen(screen, szTable, nRivals, self.iActivePlayer)
		if False:
		# for iLoopVC in range(gc.getNumVictoryInfos()):
		# LOQ 2005-09-19: end
			victory = gc.getVictoryInfo(iLoopVC)
	...
	def handleInput (self, inputClass):
		if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_LISTBOX_ITEM_SELECTED):
			...
			if (inputClass.getFunctionName() == self.VC_TAB_ID):
				...
				# LOQ 2005-09-19: remove Space Ship Screen (original commented out)
				self.iScreen = VICTORY_CONDITION_SCREEN
				self.showVictoryConditionScreen()
				# self.iScreen = SPACESHIP_SCREEN
				# self.showSpaceshipScreen()
			elif (inputClass.getFunctionName() == self.UN_RESOLUTION_TAB_ID):
				# LOQ 2005-09-19: remove UN Resolution Screen (original commented out)
				self.iScreen = VICTORY_CONDITION_SCREEN
				self.showVictoryConditionScreen()
