...
import CvDesertWar	# LOQ 2005-08-15

# globals
gc = CyGlobalContext()
ArtFileMgr = CyArtFileMgr()
localText = CyTranslator()
dw = CvDesertWar.CvDesertWar()	# LOQ 2005-08-15
...
class CvFinanceAdvisor:
	...
	def drawContents(self):
		...
		# LOQ 2005-08-15: Operation Barbarossa expense added
		if ( dw.isOperationBarbarossa() and self.iActiveLeader == CvDesertWar.iGermans ):
			yLocation += self.Y_SPACING
			screen.setLabel(self.getNextWidgetName(), "Background", u"<font=3>" + localText.getText("TXT_KEY_TECH_BARBAROSSA", ()) + "</font>", CvUtil.FONT_LEFT_JUSTIFY, self.X_EXPENSES + self.TEXT_MARGIN, yLocation + self.TEXT_MARGIN, self.Z_CONTROLS + self.DZ, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, self.iActiveLeader, 1)
			screen.setLabel(self.getNextWidgetName(), "Background", u"<font=3>" + unicode(dw.getExtraUnitCost()) + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_EXPENSES + self.PANE_WIDTH - self.TEXT_MARGIN, yLocation + self.TEXT_MARGIN, self.Z_CONTROLS + self.DZ, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, self.iActiveLeader, 1)
			iExpenses += dw.getExtraUnitCost()
