...
#####################################  
        ...
        def checkTurn(self, iGameTurn):
                ...
                if (((iGameTurn > con.i600BC and gc.getPlayer(0).isPlayable()) or (iGameTurn > 151+20 and not gc.getPlayer(0).isPlayable())) and iGameTurn % 20 == 15): #late start condition
        ...
        def updateBaseStability(self, iGameTurn, iPlayer):
                ...
                if (iGameTurn >= con.tBirth[iPlayer]+15):
                        ...
                        if (iGameTurn % 3 == 2):
                                ...
                                iMaxShrink = 7
                                iMaxGrowth = 3
                                iNegativeGrowth = self.getGNPnew(iPlayer)/3 - self.getGNPold(iPlayer)/3
                                iPositiveGrowth = (self.getGNPnew(iPlayer)/3 - self.getGNPold(iPlayer)/3 -3) /3
                                
                                if (iPlayer == con.iJapan or iPlayer == con.iInca): #counterbalance their stagnation due to isolation
                                        iNegativeGrowth = self.getGNPnew(iPlayer)/3 - self.getGNPold(iPlayer)/3 +1
                                if (iPlayer == con.iIndia or iPlayer == con.iChina or iPlayer == con.iJapan or iPlayer == con.iInca): #counterbalance their stagnation due to isolation
                                        iPositiveGrowth = (self.getGNPnew(iPlayer)/3 - self.getGNPold(iPlayer)/3 -2) /3
                                ...
                                if (self.getGNPnew(iPlayer) < self.getGNPold(iPlayer)):
                                ...
                                elif (self.getGNPnew(iPlayer) > self.getGNPold(iPlayer) +3):
                                        ...
                                        #self.setStability(iPlayer, self.getStability(iPlayer) -1)
                                        #print("Stability - GNP check", 0, iPlayer)
                                        pass
                                                ...
                                                if (not pPlayer.isGoldenAge()):
                                                        if ((iDifference > 11 and self.getGNPnew(iPlayer) > self.getGNPold(iPlayer)) or \
                                                            (iDifference > 6 and self.getGNPnew(iPlayer) > self.getGNPold(iPlayer) + 4)): #low wages and big growth
                                                                self.setGreatDepressionCountdown(iPlayer, 10) #10 turns
                ...
                if (self.getGreatDepressionCountdown(iPlayer) > 0):
                        ...
                        if (self.getGreatDepressionCountdown(iPlayer) == 0): #just quit
                                self.setGreatDepressionCountdown(iPlayer, -30) ##set 30 turns of immunity
                        if (self.getGreatDepressionCountdown(iPlayer) < 6): #should last at least 5 turns 
                                if ((iDifference < 5 and self.getGNPnew(iPlayer) <= self.getGNPold(iPlayer)) or iCivic3 != 17): #better wages and natural deflation, or no free market anymore
                                        self.setGreatDepressionCountdown(iPlayer, -25) ##quit from the spiral immediately and set 25 turns of immunity

                if (iGameTurn % 3 == 2):
                        self.setGNPold(iPlayer, self.getGNPnew(iPlayer))
                        self.setGNPnew(iPlayer, 0)

                if (self.getGreatDepressionCountdown(iPlayer) == 0):   #acquire only if there's no depression already and if it's not immune
                        for iLoopCiv in range(iNumPlayers):
        ...
        def onBuildingBuilt(self, iPlayer, iBuilding, city):
                ...
                if (iBuilding == con.iPalace): #palace
                        self.setStability(iPlayer, self.getStability(iPlayer) - 15 )
        ...
        def checkImplosion(self, iGameTurn):
                ...
                if (iGameTurn > con.i50AD and iGameTurn % 8 == 5):
                        for iPlayer in range(iNumPlayers):
                                        ...
                                        if (self.getStability(iPlayer) < -40): #civil war
                                                ...
                                                if (iPlayer != utils.getHumanID()):
                                                        ...
                                                        if (iGameTurn < con.i1000AD):
                                                                utils.killAndFragmentCiv(iPlayer, iIndependent, iIndependent2, iBarbarian, False)
                                                        else:
