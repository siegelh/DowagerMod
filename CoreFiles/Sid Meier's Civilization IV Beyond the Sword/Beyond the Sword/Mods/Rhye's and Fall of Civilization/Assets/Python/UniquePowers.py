...
#####################################  
        ...
        def checkTurn(self, iGameTurn):
                ...
                if (iGameTurn >= con.i1607AD+5 and ((self.getImmigrationTurnLength() != 0) or ((gc.getGame().getSorenRandNum(30, 'random') % 20) == 0 and gc.getPlayer(iAmerica).isAlive()))):
                        self.americanUP()

                if (iGameTurn >= con.i1190AD):
                #if (iGameTurn >= 0): #debug
                        if (iGameTurn == self.getLatestRazeData(0)+iMongolianTimer):
                                self.useMongolUP()
                ...
                for x in range(city.getX()-2, city.getX()+3):
                        for y in range(city.getY()-2, city.getY()+3):
                                ...
                                if (x == city.getX() and y == city.getY()):
                                        ...
                                        utils.convertPlotCulture(pCurrent, iTurkey, 100, True)
                                else:
