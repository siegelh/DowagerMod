...
################################################   
        ...
        def getFirstContactPlague( self, iCiv ):
                scriptDict = pickle.loads( gc.getGame().getScriptData() )
                return scriptDict['lFirstContactPlague'][iCiv]

        def setFirstContactPlague( self, iCiv, bNewValue ):
                scriptDict = pickle.loads( gc.getGame().getScriptData() )
                scriptDict['lFirstContactPlague'][iCiv] = bNewValue
                gc.getGame().setScriptData( pickle.dumps(scriptDict) )
...
##                self.setGenericPlagueDates(4, 340 + gc.getGame().getSorenRandNum(30, 'Variation') - 15) #1850 ca
                if (gc.getPlayer(0).isPlayable()):  #late start condition
                        self.setGenericPlagueDates(0, 140 + gc.getGame().getSorenRandNum(40, 'Variation') - 20) #400
                else:
                        self.setGenericPlagueDates(0, 80) #safe value
                self.setGenericPlagueDates(1, 220 + gc.getGame().getSorenRandNum(40, 'Variation') - 20) #1300 ca
                self.setGenericPlagueDates(2, 285 + gc.getGame().getSorenRandNum(40, 'Variation') - 20) #1650 ca
                self.setGenericPlagueDates(3, 350 + gc.getGame().getSorenRandNum(40, 'Variation') - 20) #1850 ca
        ...
        def stopPlague(self, iPlayer):
                ...
                if (self.getFirstContactPlague(iPlayer) == True):
                        self.setPlagueCountdown(iPlayer, -iImmunity-30)
