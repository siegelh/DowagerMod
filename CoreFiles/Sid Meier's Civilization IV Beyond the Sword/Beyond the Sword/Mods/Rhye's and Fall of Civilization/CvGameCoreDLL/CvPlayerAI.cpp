...
int CvPlayerAI::AI_foundValue(int iX, int iY, int iMinRivalRange, bool bStartingLoc) const
{
	...
	iValue *= settlersMaps[getID()][EARTH_Y - 1 - tempY][tempX];
	iValue /= 100;

	if (settlersMaps[getID()][EARTH_Y - 1 - tempY][tempX] <= 3)
		iValue /= 2;
	if (settlersMaps[getID()][EARTH_Y - 1 - tempY][tempX] <= 60)
		iValue /= 2;
...
int CvPlayerAI::AI_cityTradeVal(CvCity* pCity) const
{
	...
	if (GET_PLAYER(pCity->getOwnerINLINE()).getNumCities() > 12) 
		if (settlersMaps[pCity->getOwnerINLINE()][EARTH_Y -1 - pCity->getY()][pCity->getX()] < 500) { //not in core area
			iValue *= 2;
			iValue /= 3;
		}
	//Rhye - end
...
int CvPlayerAI::AI_civicValue(CivicTypes eCivic) const
{
	...
	if (iTempValue != 0)
	{
		...
		iValue += (getNumCities() * 8 * AI_getHappinessWeight(isCivic(eCivic) ? -iTempValue : iTempValue, 1)) / 100; //Rhye
	}
