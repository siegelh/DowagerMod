...
DenialTypes CvTeamAI::AI_surrenderTrade(TeamTypes eTeam, int iPowerMultiplier) const
{
	...
	//Rhye - start (4 vassals cap if already the strongest)	
...
int CvTeamAI::AI_defensivePactTradeVal(TeamTypes eTeam) const
{
	...
	else if (GC.getGameINLINE().getGameTurn() > 380) { //the last 100 turns, starting from 1920
		modifier = 160;
	}	
	...
	return ((getNumCities() + GET_TEAM(eTeam).getNumCities()) * modifier / 100); //Rhye
	//Rhye - end
}
	...
	for (iI = 0; iI < MAX_PLAYERS; iI++)
	{
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if (GET_PLAYER((PlayerTypes)iI).getTeam() == getID())
			{
				...
				if ((eAttitude <= GC.getLeaderHeadInfo(GET_PLAYER((PlayerTypes)iI).getPersonalityType()).getDefensivePactRefuseAttitudeThreshold() && iGameTurn < 380) ||
					(eAttitude <= (AttitudeTypes)std::max(0,GC.getLeaderHeadInfo(GET_PLAYER((PlayerTypes)iI).getPersonalityType()).getDefensivePactRefuseAttitudeThreshold()-1) && iGameTurn >= 380)) //1920, the last 100 turns
				//Rhye - end
				{
					return DENIAL_ATTITUDE;
				}
