...
# SPACESHIP LAUNCH BUTTON POSITIONS
######################
iLaunchButtonSize = 32
iLaunchPosX = 260 # distance from right
iLaunchPosY = 147 # distance from bottom
...
class CvMainInterface:
	...
	def interfaceScreen (self):
		...
		screen.setImageButton( "LaunchButton", "", xResolution - (iLaunchButtonSize/2) - iLaunchPosX, yResolution - (iLaunchButtonSize/2) - iLaunchPosY, iLaunchButtonSize, iLaunchButtonSize, WidgetTypes.WIDGET_LAUNCH_VICTORY, -1, -1 )
		screen.setStyle( "LaunchButton", "Button_HUDEndTurn_Style" )
		screen.setEndTurnState( "LaunchButton", "Red" )
		screen.hide( "LaunchButton" )
...
##############################################3
	...
	def redraw( self ):
		...
		if ( CyInterface().isDirty(InterfaceDirtyBits.PlotListButtons_DIRTY_BIT) == True ):
			printd("Plot List Buttons Dirty")
		...
		if ( CyInterface().isDirty(InterfaceDirtyBits.SelectionButtons_DIRTY_BIT) == True ):
			printd("Selection Buttons Dirty")
...
#			printd("Help Dirty")
	...
	def updateLaunchButton( self ):
		...
		if (CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_SHOW):
			if (gc.getGame().getGameState() == GameStateTypes.GAMESTATE_ON and gc.getGame().getWinner() == -1):
				for i in range(gc.getNumVictoryInfos()):
					if (gc.getTeam(gc.getGame().getActiveTeam()).canLaunch(i)):
						bShow = true
						break

		screen = CyGInterfaceScreen( "MainInterface", CvScreenEnums.MAIN_INTERFACE )
		if (bShow):
			screen.showEndTurn( "LaunchButton" )
		else:
			screen.hideEndTurn( "LaunchButton" )			
			...
			if (CyInterface().getInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_SELECTION):
				...
				if ( pHeadSelectedUnit.getOwner() == gc.getGame().getActivePlayer() and g_pSelectedUnit != pHeadSelectedUnit ):
					...
					for i in actions:
						...
						# Can or cannot build a starbase?
						iActionMissionType = gc.getActionInfo(i).getMissionType()
						if (iActionMissionType == MissionTypes.MISSION_BUILD):
							iActionBuildType = gc.getActionInfo(i).getMissionData()
							iBuildStarbase = CvUtil.findInfoTypeNum(gc.getBuildInfo,gc.getNumBuildInfos(),'BUILD_STARBASE')
							if (iActionBuildType == iBuildStarbase):
								print("Mission is to build a Starbase")
								FinalFrontier = CvEventInterface.getEventManager()
								if (not FinalFrontier.canBuildStarbase(pHeadSelectedUnit.plot(), 1)):
									print("Should be disabling build button")
									screen.disableMultiListButton( "BottomButtonContainer", 0, iCount, gc.getActionInfo(i).getButton() )
...
#						screen.show( szName )
	...
	def updateCityScreen( self ):
		...
		screen.addScrollPanel( "BonusBack1", u"", xResolution - 191, 70, 184, 100, PanelStyles.PANEL_STYLE_EXTERNAL )
		...
		screen.addScrollPanel( "BonusBack2", u"", xResolution - 125, 70, 205, 100, PanelStyles.PANEL_STYLE_EXTERNAL )
...
#		screen.hide( "AssignBuildingButton" )
		...
		if ( CyInterface().isCityScreenUp() ):
			if ( pHeadSelectedCity ):
				...
				screen.addTableControlGFC( "BuildingListTable", 3, 10, 317, 251, yResolution - 541, False, False, 32, 32, TableStyles.TABLE_STYLE_STANDARD )
				...
				screen.setTableColumnHeader( "BuildingListTable", 0, u"", 158 )
				screen.setTableColumnHeader( "BuildingListTable", 1, u"", 105 )
				screen.setTableColumnHeader( "BuildingListTable", 2, u"", 2 )
				...
				printd("Showing Final Frontier City Screen IFace stuff")
...
# Removed
				...
				for i in range( gc.getNumBuildingInfos() ):
					if (pHeadSelectedCity.getNumBuilding(i) > 0):
						for k in range(pHeadSelectedCity.getNumBuilding(i)):
							...
							if (pHeadSelectedCity.getNumActiveBuilding(i)):
	...
	def updateInfoPaneStrings( self ):
		...
		screen.addTableControlGFC( "SelectedUnitText", 3, 10, yResolution - 109, 223, 102, False, False, 32, 32, TableStyles.TABLE_STYLE_STANDARD )
		screen.setStyle( "SelectedUnitText", "Table_EmptyScroll_Style" )
		screen.hide( "SelectedUnitText" )
		screen.hide( "SelectedUnitLabel" )
		
		screen.addTableControlGFC( "SelectedCityText", 3, 10, yResolution - 139, 223, 128, False, False, 32, 32, TableStyles.TABLE_STYLE_STANDARD )
		...
		if (pHeadSelectedCity):
			...
			screen.setTableColumnHeader( "SelectedCityText", 0, u"", 161 )
