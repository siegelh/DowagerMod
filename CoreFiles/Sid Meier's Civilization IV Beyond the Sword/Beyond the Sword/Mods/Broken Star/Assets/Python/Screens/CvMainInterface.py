...
class CvMainInterface:
	...
	def interfaceScreen (self):
		...
		# Advisor Buttons...
		#B_BROKENSTAR
		screen.setImageButton( "BuySellUnitsButton", "", iBtnX, iBtnY, iBtnWidth, iBtnWidth, WidgetTypes.WIDGET_GENERAL, 669, 669 )
		screen.setStyle( "BuySellUnitsButton", "Button_HUDBuySellUnit_Style" )
		screen.hide( "BuySellUnitsButton" )
		iBtnX += iBtnAdvance
		#E_BROKENSTAR
	...
	def updateMiscButtons( self ):
		...
		if ( CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_HIDE_ALL or CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_MINIMAP_ONLY ):
			...
			#B_BROKENSTAR
			self.moveToFrontBuySellUnitsButton()
			#E_BROKENSTAR
					...
					if ( bHandled == False ):
						...
						if (eOrderNodeType  == OrderTypes.ORDER_TRAIN ):
							...
							if(gc.getProjectInfo(CyInterface().getOrderNodeData1(i)).isSpaceship()):
								modelType = 0
								screen.addSpaceShipWidgetGFC("InterfaceUnitModel", 190, yResolution - 138, 103, 132, CyInterface().getOrderNodeData1(i), modelType, WidgetTypes.WIDGET_HELP_SELECTED, 0, -1)
							else:
		...
		if (not CyEngine().isGlobeviewUp() and pHeadSelectedCity):
			...
			if (CyInterface().getInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_SELECTION):
				...
				if ( pHeadSelectedUnit.getOwner() == gc.getGame().getActivePlayer() and g_pSelectedUnit != pHeadSelectedUnit ):
					...
					#B_BROKENSTAR			
					screen.appendMultiListButton( "BottomButtonContainer", "Art/Interface/Buttons/Civics/Police_State.dds", 0, WidgetTypes.WIDGET_GENERAL, 611,611, False )
					
					disableBuyPromotionButton = 1
					for i in range(gc.getNumPromotionInfos()):
						if (pHeadSelectedUnit.canAcquirePromotion(i) == true and gc.getPromotionInfo(i).isLeader() == false):
							disableBuyPromotionButton = 0
							
					if disableBuyPromotionButton == 1:
						screen.disableMultiListButton( "BottomButtonContainer", 0, iCount, gc.getActionInfo(i).getButton() )
	...
	def handleInput (self, inputClass):
		#B_BROKENSTAR
		if ("""inputClass.getNotifyCode() == 0""" and inputClass.getData1() == 669 and inputClass.getData2() == 669):
				CvScreensInterface.showBuySellUnits()
				
		if ("""inputClass.getNotifyCode() == 11""" and inputClass.getData1() == 611 and inputClass.getData2() ==611):
				
				iNetMessage = BrokenStar.BrokenStar().iNetMessage_BuyPromotion
				iPlayer = CyGame().getActivePlayer()
				pUnit = CyInterface().getHeadSelectedUnit()
				iUnitID = pUnit.getID()
				
				CyMessageControl().sendModNetMessage(iNetMessage,iPlayer,iUnitID,-1,-1)
#				BrokenStar.BrokenStar().promoteUnit()
		#E_BROKENSTAR	
		return 0
		
	def hideBuySellUnitsButton(self):
		screen = CyGInterfaceScreen( "MainInterface", CvScreenEnums.MAIN_INTERFACE )
		screen.hide( "BuySellUnitsButton" )
		return
		
	def showBuySellUnitsButton(self):
		screen = CyGInterfaceScreen( "MainInterface", CvScreenEnums.MAIN_INTERFACE )
		screen.show( "BuySellUnitsButton" )
		return
		
	def moveToFrontBuySellUnitsButton(self):
		screen = CyGInterfaceScreen( "MainInterface", CvScreenEnums.MAIN_INTERFACE )
		screen.moveToFront( "BuySellUnitsButton" )
		return
