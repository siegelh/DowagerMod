...
class CvVictoryScreen:
	...
	def showVictoryConditionScreen(self):
		...
		# Mutually Assured Destruction Victory
		for player in PyHelpers.PyGame().getCivPlayerList():
			if player.getID() == 9:
				for unit in player.getUnitList():
					if(unit.getUnitType() == gc.getInfoTypeForString("UNIT_UNARMED_NUKE") or unit.getUnitType() == gc.getInfoTypeForString("UNIT_NUKE")):
						iRemNukes += 1	
		...
		for iLoopVC in range(gc.getNumVictoryInfos()):
			...
			if gc.getGame().isVictoryValid(iLoopVC):
				...
				if(victory.getTextKey() == "TXT_KEY_VICTORY_ARESENAL"):
					iRow = screen.appendTableRow(szTable)
					screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_ARSENAL_PEDIA", (g_iNukesforVictory,)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					screen.appendTableRow(szTable)
					screen.setTableText(szTable, 4, iRow, localText.getText("TXT_KEY_VICTORY_ARSENAL_REM_NUKES", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					screen.setTableText(szTable, 5, iRow, unicode(iRemNukes), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					for player in PyHelpers.PyGame().getCivPlayerList():
						print("Player ID: %d" %(player.getID()))
						print("Monies!!!" %())
						if player.getID() < 8:
							...
							for unit in player.getUnitList():
								if(unit.getUnitType() == gc.getInfoTypeForString("UNIT_NUKE")):
									iNumNukes += 1
							print("%s has %d nukes" %(szName, iNumNukes))
							screen.setTableText(szTable, 2, (iRow + iPlayerID), localText.getText("TXT_KEY_VICTORY_ARSENAL_SCREEN", (szName,iNumNukes)), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
							screen.setTableText(szTable, 3, (iRow + iPlayerID), unicode(iNumNukes), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
							screen.appendTableRow(szTable)
...
#					bEntriesFound = True
				...
				if (victory.isConquest()):
					iRow = screen.appendTableRow(szTable)
					screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_ELIMINATE_ALL_BROKEN_STAR", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					screen.setTableText(szTable, 2, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_RIVALS_LEFT_BROKEN_STAR", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
