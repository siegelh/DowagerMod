...
#Defense Variables
#bIsInAdvancedStart = -1
iRoundFlashTimer = 1
#szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_ONE"
#szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_ONE_TITLE"
#bTurnTimerOff = 1
#iTurnTimerTrigger = 1
#iTurnTimer = 0
...
xTimerLocation = - 50
yTimerLocation = 75
yTimerTitleLocation = 65
yEnemyCounterLocation = 80
yRoundCounterLocation = 50
...
#			szName = "CorporationButton" + str(i)
#			screen.setImageButton( szName, gc.getCorporationInfo(i).getTechButton(), 0, 0, 32, 32, WidgetTypes.WIDGET_RESEARCH, gc.getCorporationInfo(i).getTechPrereq(), -1 )
#			screen.hide( szName )
...
#		screen.hide( "ResearchBar" )
		...
		xResolution = screen.getXResolution()
		yResolution = screen.getYResolution()
		Defense = CvEventInterface.getEventManager()
		screen.setText( "roundOverlay", "Background", "<font=4>" + localText.getText( Defense.szRoundOverlay , ()) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, xResolution/2, yResolution/2-25, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		screen.hide( "roundOverlay" )
		screen.setText( "titleOverlay", "Background", "<font=4>" + localText.getText( Defense.szRoundTitleOverlay , ()) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, xResolution/2, yResolution/2+25, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		screen.hide( "titleOverlay" )

		# *********************************************************************************
		# Timer Title
		# *********************************************************************************
		
		screen.setText( "TimerTitle", "Background", "<font=3>" + localText.getText( "TXT_KEY_DEFENSE_TIME_REMAINING" , (00, )) + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, xResolution + xTimerLocation, yTimerTitleLocation, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		screen.hide( "TimerTitle" )

		# *********************************************************************************
		# Enemy Counter
		# *********************************************************************************

		screen.setText( "EnemyCounter", "Background", "<font=3>" + localText.getText( "TXT_KEY_DEFENSE_ENEMY_COUNTER" , ( )) + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, xResolution + xTimerLocation, yEnemyCounterLocation, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		screen.hide( "EnemyCounter" )
		
		# *********************************************************************************
		# Round Counter
		# *********************************************************************************		

		screen.setText( "RoundCounter", "Background", "<font=3>" + localText.getText( Defense.szRoundOverlay , ( )) + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, xResolution + xTimerLocation, yRoundCounterLocation, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		screen.hide( "RoundCounter" )
	...
	def updateScreen(self):
		...
		Defense = CvEventInterface.getEventManager()
#		global bIsInAdvancedStart
#		global bTurnTimerOff
#		global iTurnTimerTrigger
#		global iTurnTimer
		
		if Defense.bIsInAdvancedStart == 0 and Defense.bTurnTimerOff == 1:
#			if Defense.bHasSetGold == 0:
#				gc.getPlayer( 0 ).setGold( Defense.iCurrentAdvancedStartPoints )
#				Defense.bHasSetGold = 1
			Defense.bTurnTimerOff = 0
			Defense.iTurnTimer = 0		
			Defense.doMapUnitPowerModification()
			if PyPlayer( 0 ).getNumUnits( ) > 6:
				Defense.iTurnTimerTrigger = PyPlayer( 0 ).getNumUnits( ) * 16
			else:
				Defense.iTurnTimerTrigger = 96
		
		if Defense.bIsInAdvancedStart == 0 and Defense.bTurnTimerOff == 0:
			if Defense.iTurnTimer >= Defense.iTurnTimerTrigger:
						...
						for j in range( gc.getNumCorporationInfos() ):
							if ( not bDone ):
								if (gc.getCorporationInfo(j).getTechPrereq() == i):
									if not (gc.getGame().isCorporationFounded(j)):
										szName = "CorporationButton" + str(j)
										bDone = True
...
#					screen.show( "ResearchBar" )
	...
	def updateScoreStrings( self ):
		...
		if (CyInterface().isScoresVisible() and not CyInterface().isCityScreenUp() and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_ADVANCED_START and CyEngine().isGlobeviewUp() == false):
...
#		global bIsInAdvancedStart
		global iRoundFlashTimer
#		global bTurnTimerOff

		iNumPlots = gc.getMap().numPlots()		

		Defense.iCurrentAdvancedStartPoints = gc.getPlayer( 0 ).getAdvancedStartPoints( )

		if CyInterface( ).isInAdvancedStart( ):
			Defense.bIsInAdvancedStart = 1
			iRoundFlashTimer = 1000
		
#			if Defense.bPlotReveal == 0:
#				Defense.bPlotReveal = 1
#				for i in range( iNumPlots ):
#					gc.getMap().plotByIndex( i ).changeVisibilityCount(0 , 1, InvisibleTypes.NO_INVISIBLE)

		else:
			Defense.bIsInAdvancedStart = 0
		
		if Defense.bIsInAdvancedStart == 0 and iRoundFlashTimer > 0:
			iRoundFlashTimer +=-1
			self.doShowOverlay( )
		else:
			screen.hide( "roundOverlay" )
	...
	def doShowOverlay( self ):
		Defense = CvEventInterface.getEventManager()
		screen = CyGInterfaceScreen( "MainInterface", CvScreenEnums.MAIN_INTERFACE )
		xResolution = screen.getXResolution()
		yResolution = screen.getYResolution()
		self.doSetOverlayText()
		screen.setText( "roundOverlay", "Background", "<font=4>" + localText.getText( Defense.szRoundOverlay , ()) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, xResolution/2, yResolution/2-25, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		screen.setText( "titleOverlay", "Background", "<font=4>" + localText.getText( Defense.szRoundTitleOverlay , ()) + "</font>", CvUtil.FONT_CENTER_JUSTIFY, xResolution/2, yResolution/2+25, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		screen.show( "roundOverlay" )
		screen.show( "titleOverlay" )
	
	def doShowEnemyCounter( self, iMobGoal, iMobCounted ):
		Defense = CvEventInterface.getEventManager()
#		iMobsRemaining = Defense.iMobsKilled
#		iMobsRemaining = ( Defense.iTotalMobs - Defense.iMobsKilled )
		iMobsRemaining = ( iMobGoal - iMobCounted )
		screen = CyGInterfaceScreen( "MainInterface", CvScreenEnums.MAIN_INTERFACE )
		xResolution = screen.getXResolution()
		yResolution = screen.getYResolution()
		
		szString = "<font=3>" + localText.getText( "TXT_KEY_DEFENSE_ENEMY_COUNTER", ()) + " %d</font>" %( iMobsRemaining )
		screen.setText( "EnemyCounter", "Background", szString, CvUtil.FONT_RIGHT_JUSTIFY, xResolution + xTimerLocation, yEnemyCounterLocation, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		screen.show( "EnemyCounter" )

	def doHideEnemyCounter( self ):
		screen = CyGInterfaceScreen( "MainInterface", CvScreenEnums.MAIN_INTERFACE )
		
		screen.hide( "EnemyCounter" )	

	def doShowTurnTimer( self, iCurrentTime, iTimeTrigger ):
		iSecondsRemaining = (( iTimeTrigger - iCurrentTime ) // 4) % 60
		iMinutesRemaining = (( iTimeTrigger - iCurrentTime ) // 4) // 60
		screen = CyGInterfaceScreen( "MainInterface", CvScreenEnums.MAIN_INTERFACE )
		xResolution = screen.getXResolution()
		yResolution = screen.getYResolution()
		Defense = CvEventInterface.getEventManager()
		
		if iSecondsRemaining > 9:
			szSecondsRemaining = u"%d" %(iSecondsRemaining)#str(iSecondsRemaining)
		else:
			szSecondsRemaining = u"0%d" %(iSecondsRemaining)#"0" + str(iSecondsRemaining)
		
		szString = "<font=3>" + localText.getText( "TXT_KEY_DEFENSE_TIME_REMAINING", ()) + " %d:%s</font>" %(iMinutesRemaining, szSecondsRemaining)
		screen.addPanel( "MainPanalBackground", u"", u"", True, False, xResolution + xTimerLocation - 170, yRoundCounterLocation - 5, 180, 60, PanelStyles.PANEL_STYLE_HUD_HELP )

		screen.setText( "TimerTitle", "Background", szString, CvUtil.FONT_RIGHT_JUSTIFY, xResolution + xTimerLocation , yTimerTitleLocation, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )	
		screen.setText( "RoundCounter", "Background", "<font=3>" + localText.getText( Defense.szRoundOverlay , ( )) + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, xResolution + xTimerLocation, yRoundCounterLocation, -0.3, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )

		screen.show( "MainPanalBackground" )
		screen.show( "RoundCounter" )
		screen.show( "TimerTitle" )

	
	def doHideTurnTimer( self ):
		screen = CyGInterfaceScreen( "MainInterface", CvScreenEnums.MAIN_INTERFACE )

		screen.hide( "MainPanalBackground" )		
		screen.hide( "TimerTitle" )
		screen.hide( "RoundCounter" )



	def	doSetOverlayText( self ):
		Defense = CvEventInterface.getEventManager()
					
		if Defense.iGameLevel == 2: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_TWO"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_TWO_TITLE"
		if Defense.iGameLevel == 3: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_THREE"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_THREE_TITLE"
		if Defense.iGameLevel == 4: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_FOUR"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_FOUR_TITLE"
		if Defense.iGameLevel == 5: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_FIVE"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_FIVE_TITLE"
		if Defense.iGameLevel == 6: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_SIX"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_SIX_TITLE"
		if Defense.iGameLevel == 7: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_SEVEN"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_SEVEN_TITLE"
		if Defense.iGameLevel == 8: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_EIGHT"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_EIGHT_TITLE"
		if Defense.iGameLevel == 9: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_NINE"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_NINE_TITLE"
		if Defense.iGameLevel == 10: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_TEN"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_TEN_TITLE"
		if Defense.iGameLevel == 11: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_ELEVEN"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_ELEVEN_TITLE"
		if Defense.iGameLevel == 12: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_TWELVE"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_TWELVE_TITLE"
		if Defense.iGameLevel == 13: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_THIRTEEN"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_THIRTEEN_TITLE"
		if Defense.iGameLevel == 14: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_FOURTEEN"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_FOURTEEN_TITLE"
		if Defense.iGameLevel == 15: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_FIFTEEN"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_FIFTEEN_TITLE"
		if Defense.iGameLevel == 16: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_SIXTEEN"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_SIXTEEN_TITLE"
		if Defense.iGameLevel == 17: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_SEVENTEEN"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_SEVENTEEN_TITLE"
		if Defense.iGameLevel == 18: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_EIGHTEEN"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_EIGHTEEN_TITLE"
		if Defense.iGameLevel == 19: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_NINETEEN"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_NINETEEN_TITLE"
		if Defense.iGameLevel == 20: 
			Defense.szRoundOverlay = "TXT_KEY_DEFENSE_ROUND_TWENTY"
			Defense.szRoundTitleOverlay = "TXT_KEY_DEFENSE_ROUND_TWENTY_TITLE"

	def isInCombat( self ):
		lUnits = PyPlayer( 0 ).getUnitList()
