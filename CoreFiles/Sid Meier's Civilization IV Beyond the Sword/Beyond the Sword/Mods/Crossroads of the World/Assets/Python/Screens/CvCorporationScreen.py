...
class CvCorporationScreen:
	...
	def drawCorporationInfo(self):
		...
		for i in range(gc.getNumCorporationInfos()):
			...
			if gc.getGame().getCorporationGameTurnFounded(i) >= 0:
				...
				screen.setImageButton(szButtonName, gc.getCorporationInfo(i).getButton(), self.X_CORPORATION_AREA + xLoop - self.BUTTON_SIZE/2, self.Y_CORPORATION_AREA + self.Y_CORPORATION - self.BUTTON_SIZE/2, self.BUTTON_SIZE, self.BUTTON_SIZE, WidgetTypes.WIDGET_GENERAL, -1, -1)
	#		szName = self.getCorporationTextName(i)
	#		szLabel = u"<font=2>" + gc.getCorporationInfo(i).getDescription() + u"</font>"
	#		screen.setText(szName, szArea, szLabel, CvUtil.FONT_CENTER_JUSTIFY, xLoop + self.X_CORPORATION_AREA, self.Y_CORPORATION_AREA + self.Y_CORPORATION_NAME, 2*self.DZ, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
	...
	def CorporationScreenButton( self, inputClass ):	
		if ( inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED ) :
			if (inputClass.getID() == gc.getNumCorporationInfos() or gc.getGame().getCorporationGameTurnFounded(inputClass.getID()) >= 0) :
