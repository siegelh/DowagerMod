...
class CvGameUtils:
	...
	def cannotTrain(self,argsList):
		...
		for iCorpLoop in range(gc.getNumCorporationInfos()):
			...
			# Is this unit is the executive for iCorpType
			if (pUnitInfo.getCorporationSpreads(iCorpLoop) > 0):
				
				# Is the player trying to build the executive in a city which isn't its HQ?
				if (not pCity.isHeadquartersByType(iCorpLoop)):
