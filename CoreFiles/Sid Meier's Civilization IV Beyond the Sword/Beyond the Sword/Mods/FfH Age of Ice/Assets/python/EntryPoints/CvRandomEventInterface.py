...
def canTriggerAmuriteOnly(argsList):
...
def canTriggerNonCaptialOnly(argsList):
	iEvent = argsList[0]
...
def canTriggerHero(argsList):
	kTriggeredData = argsList[0]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	if pPlayer.getCivilizationType() != gc.getInfoTypeForString('CIVILIZATION_AMURITES'):
		return False
	if pPlayer.getUnitClassCount(gc.getInfoTypeForString('UNITCLASS_BELENUS')) > 0:
		return False
	if pPlayer.getUnitClassCount(gc.getInfoTypeForString('UNITCLASS_BELENUS_MACEMAN')) > 0:
		return False
	if pPlayer.getUnitClassCount(gc.getInfoTypeForString('UNITCLASS_BELENUS_PIKEMAN')) > 0:
		return False
	if pPlayer.getUnitClassCount(gc.getInfoTypeForString('UNITCLASS_CAERBULIN')) > 0:
		return False
	if pPlayer.getUnitClassCount(gc.getInfoTypeForString('UNITCLASS_CAERBULIN_ASSASSIN')) > 0:
		return False
	if pPlayer.getUnitClassCount(gc.getInfoTypeForString('UNITCLASS_CAERBULIN_RANGER')) > 0:
		return False
	if pPlayer.getUnitClassCount(gc.getInfoTypeForString('UNITCLASS_EPONA')) > 0:
		return False
	if pPlayer.getUnitClassCount(gc.getInfoTypeForString('UNITCLASS_EPONA_SORCERESS')) > 0:
		return False
	if CyGame().getUnitCreatedCount(gc.getInfoTypeForString('UNIT_BELENUS')) + CyGame().getUnitCreatedCount(gc.getInfoTypeForString('UNIT_CAERBULIN')) + CyGame().getUnitCreatedCount(gc.getInfoTypeForString('UNIT_EPONA')) > 2:
		return False
	return True

def canApplyBelenus(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	if CyGame().getUnitCreatedCount(gc.getInfoTypeForString('UNIT_BELENUS')) > 0:
		return False
	return True

def canApplyCaerbulin(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	if CyGame().getUnitCreatedCount(gc.getInfoTypeForString('UNIT_CAERBULIN')) > 0:
		return False
	return True

def canApplyEpona(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	if CyGame().getUnitCreatedCount(gc.getInfoTypeForString('UNIT_EPONA')) > 0:
		return False
	return True

def canTriggerSpy(argsList):
	kTriggeredData = argsList[0]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	if pPlayer.getCivilizationType() != gc.getInfoTypeForString('CIVILIZATION_AMURITES'):
		return False
	...
	for iPlayer2 in range(gc.getMAX_PLAYERS()):
		...
		if pPlayer2.isAlive():
			if pPlayer2.getCivilizationType() == gc.getInfoTypeForString('CIVILIZATION_DOVIELLO'):
				iTeam2 = pPlayer2.getTeam()
				if eTeam.isAtWar(iTeam2):
					return True
	return False

def doDisciple1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
...
def canTriggerGodslayerBladeDone(argsList):
	...
	iGodslayerBlade = gc.getInfoTypeForString('PROMOTION_GODSLAYER_BLADE')
	for pUnit in py.getUnitList():
		if pUnit.isHasPromotion(iGodslayerBlade):
			return true
	
	return false

def canTriggerGodslayerCrossguardDone(argsList):
	...
	py = PyPlayer(kTriggeredData.ePlayer)
	iGodslayerBlade = gc.getInfoTypeForString('PROMOTION_GODSLAYER_CROSSGUARD')
	for pUnit in py.getUnitList():
		if pUnit.isHasPromotion(iGodslayerBlade):
			return true
...
def canTriggerGodslayerHiltDone(argsList):
	...
	py = PyPlayer(kTriggeredData.ePlayer)
	iGodslayerBlade = gc.getInfoTypeForString('PROMOTION_GODSLAYER_HILT')
	for pUnit in py.getUnitList():
		if pUnit.isHasPromotion(iGodslayerBlade):
			return true
...
def canTriggerGodslayerReforgeDone(argsList):
...
def canApplyOgre3(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	if pPlayer.getBuildingClassCount(gc.getInfoTypeForString('BUILDINGCLASS_TROPHY_HALLS')) == 0:
		return False
	return True

def doOgre1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	pCity = pPlayer.getCapitalCity()
	newUnit = pPlayer.initUnit(gc.getInfoTypeForString('UNIT_OGRE'), pCity.getX(), pCity.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_SOUTH)

def doSeer1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
...
def doSeer2(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]

	pPlot = CyMap().plot(49,3)
	pPlot.setBonusType(gc.getInfoTypeForString('BONUS_WINE'))
	CyInterface().addMessage(pPlot.getOwner(),True,25,CyTranslator().getText("TXT_KEY_POPUP_GIFT_WINE",()),'AS2D_DISCOVERBONUS',1,'Art/Interface/Buttons/Worldbuilder/Wine.dds',ColorTypes(8),pPlot.getX(),pPlot.getY(),True,True)

def doSeer3(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
...
def doSeer4(argsList):
	...
	for i in range(pPlayer.getNumCities()):
		...
		if pCity.getNumRealBuilding(gc.getInfoTypeForString('BUILDING_ELDER_COUNCIL')) == 0:
			CyInterface().addMessage(pCity.getOwner(),True,25,CyTranslator().getText("TXT_KEY_POPUP_GIFT_ELDER_COUNCIL",()),'AS2D_DISCOVERBONUS',1,'Art/Interface/Buttons/Buildings/Elder Council.dds',ColorTypes(8),pCity.getX(),pCity.getY(),True,True)
			pCity.setNumRealBuilding(gc.getInfoTypeForString('BUILDING_ELDER_COUNCIL'), 1)

def doSeer5(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
			...
			if pPlot.isOwned():
				if pPlot.getBonusType(-1) == iCopper:
					if pPlot.getImprovementType() == iMine:
						if gc.getPlayer(pPlot.getOwner()).getCivilizationType() == iDoviello:
...
def doSpy2(argsList):
	...
	for iPlayer in range(gc.getMAX_PLAYERS()):
		...
		if pPlayer.isAlive():
			if pPlayer.getCivilizationType() == gc.getInfoTypeForString('CIVILIZATION_DOVIELLO'):
				pCity = pPlayer.getCity(CyGame().getSorenRandNum(pPlayer.getNumCities(), "Spy2"))
				CyInterface().addMessage(kTriggeredData.ePlayer,True,25,CyTranslator().getText("TXT_KEY_POPUP_DOVIELLO_FIRE",()),'AS2D_PILLAGE',1,'Art/Interface/Buttons/Actions/Pillage.dds',ColorTypes(8),pCity.getX(),pCity.getY(),True,True)
				for iBuilding in range(gc.getNumBuildingInfos()):
					if pCity.getNumRealBuilding(iBuilding) == 1:
						if CyGame().getSorenRandNum(100, "Spy2") <= 50:
...
def doWeddingFeud3(argsList):
	...
	if destPlayer.isHuman():
		...
		gc.getTeam(destPlayer.getTeam()).declareWar(player.getTeam(), false)
...
def applySaltpeter(argsList):
	...
	for loopPlot in listPlots:
		...
		CyInterface().addMessage(kTriggeredData.ePlayer, false, gc.getEVENT_MESSAGE_TIME(), "", "", InterfaceMessageTypes.MESSAGE_TYPE_INFO, None, gc.getInfoTypeForString("COLOR_WHITE"), loopPlot[1].getX(), loopPlot[1].getY(), true, true)
...
def getHelpInterstate(argsList):
	...
	szHelp = localText.getText("TXT_KEY_UNIT_MOVEMENT", (1, gc.getRouteInfo(CvUtil.findInfoTypeNum(gc.getRouteInfo,gc.getNumRouteInfos(),'ROUTE_ROAD')).getText()))	
...
def canTriggerGreedDone(argsList):
	...
	trigger = gc.getEventTriggerInfo(kTriggeredData.eTrigger)
	kOrigTriggeredData = player.getEventOccured(trigger.getPrereqEvent(0))
	otherPlayer = gc.getPlayer(kOrigTriggeredData.eOtherPlayer)
	plot = gc.getMap().plot(kOrigTriggeredData.iPlotX, kOrigTriggeredData.iPlotY)
...
def getHelpHostileTakeover1(argsList):
	...
	for iBonus in listResources:
		if not bFirst:
			...
			bFirst = false
		szList += u"[COLOR_HIGHLIGHT_TEXT]" + gc.getBonusInfo(iBonus).getText() + u"[COLOR_REVERT]"
