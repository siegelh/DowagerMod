...
#FfH: Added by Kael
def defeatDovielloFunc(argsList):
	iButtonId = argsList[0]
	iData1 = argsList[1]
	iData2 = argsList[2]
	iData3 = argsList[3]
	...
	if iButtonId == 0:
		pDovPlayer = gc.getPlayer(iData1)
		eDovTeam = gc.getTeam(pDovPlayer.getTeam())
		iAmurTeam = gc.getPlayer(0).getTeam()
		iIllTeam = gc.getPlayer(1).getTeam()
		iBarbTeam =  gc.getPlayer(gc.getBARBARIAN_PLAYER()).getTeam()
		for iPlayer in range(gc.getMAX_PLAYERS()):
			for iTeam in range(gc.getMAX_TEAMS()):
				gc.getTeam(gc.getPlayer(iPlayer).getTeam()).setPermanentWarPeace(iTeam, False)
		eDovTeam.makePeace(iAmurTeam)
		eDovTeam.signOpenBorders(iAmurTeam)
		eDovTeam.declareWar(iIllTeam, False, WarPlanTypes.WARPLAN_LIMITED)
		eDovTeam.declareWar(iBarbTeam, False, WarPlanTypes.WARPLAN_LIMITED)
		pDovPlayer.AI_setAttitudeExtra(0, 100)
		for iPlayer in range(gc.getMAX_PLAYERS()):
			for iTeam in range(gc.getMAX_TEAMS()):
				gc.getTeam(gc.getPlayer(iPlayer).getTeam()).setPermanentWarPeace(iTeam, True)
		pyDovPlayer = PyPlayer(iData1)
		apUnitList = pyDovPlayer.getUnitList()
		for pUnit in apUnitList:
			if pUnit.getUnitType() == gc.getInfoTypeForString('UNIT_GODSLAYER_CROSSGUARD'):
				newUnit = gc.getPlayer(0).initUnit(pUnit.getUnitType(), iData2, iData3, UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_NORTH)
				CyInterface().selectUnit(newUnit, true, true, true)
				pUnit.kill(False,0)

def defeatStagFunc(argsList):
	iButtonId = argsList[0]
	iData1 = argsList[1]
	iData2 = argsList[2]
	iData3 = argsList[3]
	...
	if iButtonId == 0:
		...
		for i in range (CyMap().numPlots()):
			...
			if pPlot.getNumUnits() == 0:
				if (pPlot.isOwned() == False and pPlot.getTerrainType() != gc.getInfoTypeForString('TERRAIN_COAST') and pPlot.isPeak() == False):
					listPlots.append(i)
		if len(listPlots) > 0:
			iRnd = CyGame().getSorenRandNum(len(listPlots), "Stag Jump")
			pPlot = CyMap().plotByIndex(listPlots[iRnd])
			bPlayer = gc.getPlayer(gc.getBARBARIAN_PLAYER())
			newUnit = bPlayer.initUnit(gc.getInfoTypeForString('UNIT_STAG'), pPlot.getX(), pPlot.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_NORTH)
		szText = CyTranslator().getText("TXT_KEY_POPUP_STAG_SPARE_FEEDBACK",())
	if iButtonId == 1:
		pPlayer = gc.getPlayer(iData1)
		pCity = pPlayer.getCapitalCity()
		pCity.setNumRealBuilding(gc.getInfoTypeForString('BUILDING_STAG_HIDE'), 1)
		szText = CyTranslator().getText("TXT_KEY_POPUP_STAG_KILL_FEEDBACK",())
...
def pullHiltFunc(argsList):
	...
	if iButtonId == 0:
		iCrater = gc.getInfoTypeForString('FEATURE_CRATER')
		iVolcano = gc.getInfoTypeForString('FEATURE_VOLCANO')
		pPlot2 = CyMap().plot(iData1,iData2)
		for i in range (CyMap().numPlots()):
			pPlot = CyMap().plotByIndex(i)
			if pPlot.getFeatureType() == iVolcano:
				pPlot.setFeatureType(iCrater, 0)
		szTitle = CyGameTextMgr().getTimeStr(CyGame().getGameTurn(), false)
		popup = PyPopup.PyPopup(-1)
		popup.setHeaderString(szTitle)
		popup.setBodyString(CyTranslator().getText("TXT_KEY_POPUP_VOLCANOS_EXPLODE",()))
		popup.launch(true, PopupStates.POPUPSTATE_QUEUED)
		point = pPlot2.getPoint()
...
def upgradeUnitFunc(argsList):
	iButtonId = argsList[0]
	iPlayer = argsList[1]
	iOldUnit = argsList[2]
	iNewUnit1 = argsList[3]
	iNewUnit2 = argsList[4]
	szText = argsList[5]
	bOption1 = argsList[6]
	bOption2 = argsList[7]
	...
	for pUnit in pyPlayer.getUnitList():
		if pUnit.getUnitType() == iOldUnit:
			newUnit = pPlayer.initUnit(iNewUnit, pUnit.getX(), pUnit.getY(), UnitAITypes.NO_UNITAI, DirectionTypes.DIRECTION_NORTH)
