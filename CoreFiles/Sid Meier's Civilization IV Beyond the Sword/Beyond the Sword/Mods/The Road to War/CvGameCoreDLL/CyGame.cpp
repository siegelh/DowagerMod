...
void CyGame::setChangeCiv(int value)
{
	if(m_pGame)
	{
		m_pGame->setChangeCiv(value);
	}
}

int CyGame::getChangeCiv()
{
//	return (NULL != m_pGame ? m_pGame->getChangeCiv() : 0);
	return m_pGame->getChangeCiv();
}
// Dale - CCH: Change Civ END

// Dale - RtW: HistEvents START
void CyGame::setHistoryEvents(int value)
{
	if(m_pGame)
	{
		m_pGame->setHistoryEvents(value);
	}
}

int CyGame::getHistoryEvents()
{
	return (NULL != m_pGame ? m_pGame->getHistoryEvents() : 0);
}

void CyGame::setVichyHappened(int value)
{
	if(m_pGame)
	{
		m_pGame->setVichyHappened(value);
	}
}

int CyGame::getVichyHappened()
{
	return (NULL != m_pGame ? m_pGame->getVichyHappened() : 0);
}

void CyGame::setBordersOpen(int team1, int team2)
{
	if(m_pGame)
	{
		m_pGame->setBordersOpen(team1, team2);
	}
}
// Dale - RtW: HistEvents END

// Dale - RtW: Game Over START
void CyGame::rtwGameOver()
{
	if(m_pGame)
	{
		m_pGame->rtwGameOver();
...
void CyGame::setRandomMapWinter()
{
	if(m_pGame)
	{
		m_pGame->setRandomMapWinter();
	}
}
