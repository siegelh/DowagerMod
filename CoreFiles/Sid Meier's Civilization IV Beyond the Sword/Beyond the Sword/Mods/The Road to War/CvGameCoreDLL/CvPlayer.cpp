...
bool CvPlayer::canTrain(UnitTypes eUnit, bool bContinue, bool bTestVisible, bool bIgnoreCost) const
{
	...
	if (!bTestVisible)
	{
		...
		// Dale - RtW: Nukes
		if (GC.getGameINLINE().isNoNukes() || !GC.getGameINLINE().isNukesValid() || !(getNukesOkay()))
		{
...
void CvPlayer::setLH(int value)
{
	return GC.getInitCore().setLeader(GC.getGame().getActivePlayer(), (LeaderHeadTypes)value);
}

void CvPlayer::setTrait(int value, bool onoff)
{
	return GC.getLeaderHeadInfo(getLeaderType()).setTrait(value, onoff);
}
void CvPlayer::setTech(int value, bool onoff)
{
	return GC.getCivilizationInfo(getCivilizationType()).setTech(value, onoff);
}

bool CvPlayer::getTech(int value)
{
	return GC.getCivilizationInfo(getCivilizationType()).getTech(value);
}
void CvPlayer::setUB(int value, int unit)
{
	return GC.getCivilizationInfo(getCivilizationType()).setUB(value, unit);
}

void CvPlayer::setUU(int value, int unit)
{
	return GC.getCivilizationInfo(getCivilizationType()).setUU(value, unit);
}
// Dale - CC: Customiser END

// Dale - RtW: Vichy France START
// value1 = new civ, value2 = new player, value3 = new team, value4 = new leader
void CvPlayer::doVichyFrance(int value1, int value2, int value3, int value4)
{
	TeamTypes eNewTeam = ((TeamTypes)value3);
	PlayerTypes eNewPlayer = ((PlayerTypes)value2);
	LeaderHeadTypes eBestLeader = ((LeaderHeadTypes)value4);
	CivilizationTypes eBestCiv = ((CivilizationTypes)value1);
	if (eNewPlayer == NO_PLAYER || eNewTeam == NO_TEAM || eBestLeader == NO_LEADER || eBestCiv == NO_CIVILIZATION)
	{
		return;
	}
	CvWString szMessage = gDLL->getText("TXT_KEY_MISC_EMPIRE_SPLIT", getNameKey(), GC.getCivilizationInfo(eBestCiv).getShortDescriptionKey(), GC.getLeaderHeadInfo(eBestLeader).getTextKeyWide());
	for (int i = 0; i < MAX_CIV_PLAYERS; ++i)
	{
		if (GET_PLAYER((PlayerTypes)i).isAlive())
		{
			gDLL->getInterfaceIFace()->addMessage((PlayerTypes)i, false, GC.getDefineINT("EVENT_MESSAGE_TIME"), szMessage, "AS2D_REVOLTEND", MESSAGE_TYPE_MAJOR_EVENT, ARTFILEMGR.getInterfaceArtInfo("INTERFACE_CITY_BAR_CAPITAL_TEXTURE")->getPath());
		}
	}
	GC.getGameINLINE().addReplayMessage(REPLAY_MESSAGE_MAJOR_EVENT, getID(), szMessage, -1, -1, (ColorTypes)GC.getInfoTypeForString("COLOR_HIGHLIGHT_TEXT"));
...
void CvPlayer::doSpanishWar(int value1, int value2, int value3, int value4)
{
	CvUnit* pGiftUnit;
	TeamTypes eNewTeam = ((TeamTypes)value3);
	PlayerTypes eNewPlayer = ((PlayerTypes)value2);
	...
	for (CvUnit* pLoopUnit = firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = nextUnit(&iLoop))
	{
		...
		if ((pPlot->getX_INLINE() == 45 && pPlot->getY_INLINE() == 10) || (pPlot->getX_INLINE() == 45 && pPlot->getY_INLINE() == 15) || (pPlot->getX_INLINE() == 44 && pPlot->getY_INLINE() == 18) || (pPlot->getX_INLINE() == 44 && pPlot->getY_INLINE() == 21) || (pPlot->getX_INLINE() == 47 && pPlot->getY_INLINE() == 22) || (pPlot->getX_INLINE() == 58 && pPlot->getY_INLINE() == 17) || (pPlot->getX_INLINE() == 50 && pPlot->getY_INLINE() == 15))
		{
...
void CvPlayer::setNukesOkay(bool bValue)
{
...
bool CvPlayer::getNukesOkay() const
{
	return m_bNukesOkay;
}
// Dale - RtW: AI Autoplay END
