...
void CyPlayer::setLH(int value)
{
	if(m_pPlayer)
	{
		m_pPlayer->setLH(value);
	}
}

void CyPlayer::setTrait(int value, bool onoff)
{
	if(m_pPlayer)
	{
		m_pPlayer->setTrait(value, onoff);
	}
}

void CyPlayer::setTech(int value, bool onoff)
{
	if(m_pPlayer)
	{
		m_pPlayer->setTech(value, onoff);
	}
}

bool CyPlayer::getTech(int value)
{
	return (NULL != m_pPlayer ? m_pPlayer->getTech(value) : false);
}

void CyPlayer::setUB(int value, int unit)
{
	if(m_pPlayer)
	{
		m_pPlayer->setUB(value, unit);
	}
}

void CyPlayer::setUU(int value, int unit)
{
	if(m_pPlayer)
	{
		m_pPlayer->setUU(value, unit);
	}
}
// Dale - CC: Customiser END

// Dale - RtW: Vichy France START
void CyPlayer::doVichyFrance(int value1, int value2, int value3, int value4)
{
	if(m_pPlayer)
	{
		m_pPlayer->doVichyFrance(value1, value2, value3, value4);
	}
}

void CyPlayer::doSpanishWar(int value1, int value2, int value3, int value4)
{
	if(m_pPlayer)
	{
		m_pPlayer->doSpanishWar(value1, value2, value3, value4);
	}
}

void CyPlayer::doAnnexation(int value1)
{
	if(m_pPlayer)
	{
		m_pPlayer->doAnnexation(value1);
	}
}

void CyPlayer::setNukesOkay(bool bValue)
{
	if(m_pPlayer)
	{
		m_pPlayer->setNukesOkay(bValue);
	}
}
// Dale - RtW: Vichy France END
