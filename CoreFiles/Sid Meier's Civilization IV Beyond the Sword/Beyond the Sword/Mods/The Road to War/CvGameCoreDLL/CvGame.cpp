...
void CvGame::reset(HandicapTypes eHandicap, bool bConstructorCall)
{
	...
	// Dale - CCH: Change Civ START
	m_iChangeCiv = 1;
	// Dale - CCH: Change Civ END
	// Dale - RtW: START
	m_iHistoryEvents = 99;
	m_iVichyHappened = 0;
	m_iBitterWinter = 0;
...
void CvGame::setActivePlayer(PlayerTypes eNewValue, bool bForceHotSeat)
{
	...
	if (eOldActivePlayer != eNewValue)
	{
		...
		// Dale - CCH: Change Civ START
		if (GC.getDCM_CIV_CHANGER() && !((GC.getGameINLINE().isPbem() || GC.getGameINLINE().isHotSeat() || GC.getGameINLINE().isPitboss())))
		{
...
void CvGame::doTurn()
{
	...
	{
		GC.setDefineINT("DCM_CIV_CHANGER", 0);
	}
	// Dale - CCH: Change Civ END
...
void CvGame::read(FDataStreamBase* pStream)
{
	...
	// Dale - CCH: Change Civ START
	pStream->Read(&m_iChangeCiv);
	// Dale - CCH: Change Civ END
	// Dale - RtW: START
	pStream->Read(&m_iHistoryEvents);
	pStream->Read(&m_iVichyHappened);
	pStream->Read(&m_iBitterWinter);
	// Dale - RtW: END
	...
	// Dale - CC: Customiser START
	int trait = 0;
	int count, count2;
	for (count2 = 0; count2 < GC.getNumLeaderHeadInfos(); count2++)
	{
		for (count = 0; count < GC.getNumTraitInfos(); count++)
		{
			pStream->Read(&trait);
		    GC.getLeaderHeadInfo((LeaderHeadTypes)count2).setTrait(count, trait != 0);
		}
	}
	for (count2 = 0; count2 < GC.getNumCivilizationInfos(); count2++)
	{
		for (count = 0; count < GC.getNumBuildingClassInfos(); count++)
		{
			pStream->Read(&trait);
			GC.getCivilizationInfo((CivilizationTypes)count2).setUB(count, trait);
		}
	}
	for (count2 = 0; count2 < GC.getNumCivilizationInfos(); count2++)
	{
		for (count = 0; count < GC.getNumUnitClassInfos(); count++)
		{
			pStream->Read(&trait);
			GC.getCivilizationInfo((CivilizationTypes)count2).setUU(count, trait);
		}
	}
	// Dale - CC: Customiser END
}
...
void CvGame::write(FDataStreamBase* pStream)
{
	...
	// Dale - CCH: Change Civ START
	pStream->Write(m_iChangeCiv);
	// Dale - CCH: Change Civ END
	// Dale - RtW: START
	pStream->Write(m_iHistoryEvents);
	pStream->Write(m_iVichyHappened);
	pStream->Write(m_iBitterWinter);
	// Dale - RtW: END
	...
	for (count2 = 0; count2 < GC.getNumLeaderHeadInfos(); count2++)
	{
		for (count = 0; count < GC.getNumTraitInfos(); count++)
		{
			pStream->Write(GC.getLeaderHeadInfo((LeaderHeadTypes)count2).getTrait(count) ? 1 : 0);
		}
	}
	for (count2 = 0; count2 < GC.getNumCivilizationInfos(); count2++)
	{
		for (count = 0; count < GC.getNumBuildingClassInfos(); count++)
		{
			pStream->Write(GC.getCivilizationInfo((CivilizationTypes)count2).getUB(count));
		}
	}
	for (count2 = 0; count2 < GC.getNumCivilizationInfos(); count2++)
	{
		for (count = 0; count < GC.getNumUnitClassInfos(); count++)
		{
			pStream->Write(GC.getCivilizationInfo((CivilizationTypes)count2).getUU(count));
		}
	}
	// Dale - CC: Customiser END
}
...
// Dale - CCH: Change Civ START
void CvGame::setChangeCiv(int value)
{
	m_iChangeCiv = value;
}

int CvGame::getChangeCiv()
{
	return m_iChangeCiv;
}
// Dale - CCH: Change Civ END
// Dale - RtW: HistEvents START
void CvGame::setHistoryEvents(int value)
{
	m_iHistoryEvents = value;
}

int CvGame::getHistoryEvents()
{
	return m_iHistoryEvents;
}

void CvGame::setVichyHappened(int value)
{
	m_iVichyHappened = value;
}

int CvGame::getVichyHappened()
{
	return m_iVichyHappened;
}
// Dale - RtW: HistEvents END

// Dale - RtW: Game Over START
void CvGame::rtwGameOver()
{
	setGameState(GAMESTATE_OVER);
	return;
}
// Dale - RtW: Game Over END

// Dale - RtW: Bitter Winter START
void CvGame::setBitterWinter(bool value)
{
	int count1, count2;
	int iBitterWinterMove = 6;
	CvPlot* pPlot;
	if(value)
	{
        m_iBitterWinter++;
		for(count1 = 0; count1 < GC.getMapINLINE().getGridWidthINLINE(); count1++)
		{
			for(count2 = (GC.getMapINLINE().getGridHeightINLINE() - (m_iBitterWinter * iBitterWinterMove)); count2 < (GC.getMapINLINE().getGridHeightINLINE() - ((m_iBitterWinter - 1) * iBitterWinterMove)); count2++)
			{
				pPlot = GC.getMapINLINE().plotINLINE(count1, count2);
				{
					if (pPlot->getTerrainType() == 0)
					{
						pPlot->setTerrainType((TerrainTypes)9, true, true);
					} else if (pPlot->getTerrainType() == 1) {
						pPlot->setTerrainType((TerrainTypes)10, true, true);
					} else if (pPlot->getTerrainType() == 3) {
						pPlot->setTerrainType((TerrainTypes)11, true, true);
					} else if (pPlot->getTerrainType() == 8) {
						pPlot->setTerrainType((TerrainTypes)12, true, true);
					}
					if (pPlot->getFeatureType() == 4)
					{
						pPlot->setFeatureType((FeatureTypes)6, pPlot->getFeatureVariety());
					}
				}
			}
		}
		...
		for(count1 = 0; count1 < GC.getMapINLINE().getGridWidthINLINE(); count1++)
		{
			for(count2 = (GC.getMapINLINE().getGridHeightINLINE() - (m_iBitterWinter * iBitterWinterMove)); count2 < (GC.getMapINLINE().getGridHeightINLINE() - ((m_iBitterWinter - 1) * iBitterWinterMove)); count2++)
			{
				{
					pPlot = GC.getMapINLINE().plotINLINE(count1, count2);
					if (pPlot->getTerrainType() == 9)
					{
						pPlot->setTerrainType((TerrainTypes)0, true, true);
					} else if (pPlot->getTerrainType() == 10) {
						pPlot->setTerrainType((TerrainTypes)1, true, true);
					} else if (pPlot->getTerrainType() == 11) {
						pPlot->setTerrainType((TerrainTypes)3, true, true);
					} else if (pPlot->getTerrainType() == 12) {
						pPlot->setTerrainType((TerrainTypes)8, true, true);
					}
					if (pPlot->getFeatureType() == 6)
					{
...
void CvGame::setRandomMapWinter()
{
	int count1, count2;
	CvPlot* pPlot;
	for(count1 = 0; count1 < GC.getMapINLINE().getGridWidthINLINE(); count1++)
	{
		for(count2 = 0; count2 < GC.getMapINLINE().getGridHeightINLINE(); count2++)
		{
			{
				pPlot = GC.getMapINLINE().plotINLINE(count1, count2);
				if (pPlot->getTerrainType() == 9)
...
// Dale - RtW: Bitter Winter END

// Dale - RtW: Open Boders START
void CvGame::setBordersOpen(int team1, int team2)
{
	GET_TEAM((TeamTypes)team1).setOpenBorders((TeamTypes)team2, true);
	GET_TEAM((TeamTypes)team2).setOpenBorders((TeamTypes)team1, true);
}
// Dale - RtW: Open Borders END
