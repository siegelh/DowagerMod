...
void CvUnit::updateAirCombat(bool bQuick)
{
	...
	if (!bFinish)
	{
		...
		CvAirMissionDefinition kAirMission;
		if (getDropRange() > 0)	// Dale - RtW	DOMAIN_AIR != getDomainType())
		{
...
{
	// Dale - RtW: START
	if (getDomainType() != DOMAIN_AIR && canAirAttack() == false)
	// Dale - RtW: END
	{
		return false;
	}
...
// Dale - RtW: Airlift change START
	if (getDomainType() == DOMAIN_SEA)
	{
		return false;
	}
// Dale - RtW: Airlift change END
...
bool CvUnit::airlift(int iX, int iY)
{
	...
	// Dale - RtW: Bitter Winter fail START
	CvPlot* pPlot;
	CvWString szBuffer;
	bool bBitter = true;
	pPlot = GC.getMapINLINE().plotINLINE(iX, iY);
	if (pPlot->getTerrainType() > 8)
	{
		if (GC.getGameINLINE().getSorenRandNum(100, "Spin the dice") < 50)
		{
			szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_WINTER", getNameKey());
			gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_BOMBARD", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), pPlot->getX_INLINE(), pPlot->getY_INLINE());
			szBuffer = gDLL->getText("TXT_KEY_MISC_THEM_WINTER", getNameKey());
			gDLL->getInterfaceIFace()->addMessage(pPlot->getOwnerINLINE(), false, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_BOMBARDED", MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_WHITE"), pPlot->getX_INLINE(), pPlot->getY_INLINE(), true, true);
			bBitter = false;
		}
	}
	if(!bBitter)
	...
	// Dale - RtW: Bitter Winter fail END
...
// Dale - RtW: START
	...
	{
		if (pCity->getNumRealBuilding((BuildingTypes)iJ) > 0)
		{
			if (GC.getBuildingInfo((BuildingTypes)iJ).getDCMParaBuilding())
			{
				return true;
			}
		}
...
// Dale - RtW	if (bombardTarget(pPlot) == NULL)
// Dale - RtW	{
// Dale - RtW		return false;
// Dale - RtW	}
		...
		if (GET_PLAYER((PlayerTypes)iI).isAlive())
		{
			if (isEnemy(GET_PLAYER((PlayerTypes)iI).getTeam()) && !isInvisible(GET_PLAYER((PlayerTypes)iI).getTeam(), false, false))
			{
				for(pLoopUnit = GET_PLAYER((PlayerTypes)iI).firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = GET_PLAYER((PlayerTypes)iI).nextUnit(&iLoop))
				{
					...
					{
// Dale - RtW						if (!pLoopUnit->isMadeInterception())
						{
...
bool CvUnit::airStrike(CvPlot* pPlot)
{
	...
	// Dale - BE: Battle Effect START
	if (!pPlot->isWater() && !isAnimal() && !pDefender->isAnimal())
	{
		pPlot->setBattleFeature(true);
	}
	// Dale - BE: Battle Effect END
...
bool CvUnit::rangeStrike(int iX, int iY)
{
	...
	// Dale - BE: Battle Effects START
	bool bBWEffect = false;
	CvPlot* bBWplot = pDefender->plot();
	if (!pDefender->getDomainType() == DOMAIN_SEA && !isAnimal() && !pDefender->isAnimal())
	{
		bBWplot->setBattleFeature(true);
	}
	// Dale - BE: Battle Effects END
	...
	if (bBitter)
	{
		if (pCity != NULL)
		{
			if (pUnit != NULL)
			{
				...
				gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_AIR_ATTACKED", MESSAGE_TYPE_INFO, pUnit->getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pCity->getX_INLINE(), pCity->getY_INLINE(), true, true);
...
bool CvUnit::bombardRanged(int iX, int iY, bool sAttack)
{
	...
	if(iDefenderNum > 0)
	{
		...
		iUnitDamage = GC.getGameINLINE().getSorenRandNum(bombardRate(), "Bombard damage");//(rand() % (bombardRate()));
	}
	if (GC.getGameINLINE().getSorenRandNum(100, "Bombard Accuracy") <= getDCMBombAccuracy())
	{
		...
		gDLL->getInterfaceIFace()->addMessage(pPlot->getOwnerINLINE(), true, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_BOMBARD", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), getX_INLINE(), getY_INLINE());
	}
	if (bTarget)
	{
		// Dale - BE: Battle Effect START
		if (pLoopUnit != NULL)
		{
			if (!pPlot->isWater() && !isAnimal() && !pLoopUnit->isAnimal())
			{
				pPlot->setBattleFeature(true);
			}
		} else {
		...
		if (pCity != NULL)
		{
			if(pCity->isBombardable(this))
			{
				pCity->changeDefenseDamage(bombardRate());
				szBuffer = gDLL->getText("TXT_KEY_MISC_YOU_DEFENSES_REDUCED_TO", pCity->getNameKey(), pCity->getDefenseModifier(false), getNameKey());
				gDLL->getInterfaceIFace()->addMessage(pCity->getOwnerINLINE(), false, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_BOMBARDED", MESSAGE_TYPE_INFO, GC.getUnitInfo(getUnitType()).getButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pCity->getX_INLINE(), pCity->getY_INLINE(), true, true);
				szBuffer = gDLL->getText("TXT_KEY_MISC_ENEMY_DEFENSES_REDUCED_TO", getNameKey(), pCity->getNameKey(), pCity->getDefenseModifier(false));
				gDLL->getInterfaceIFace()->addMessage(getOwnerINLINE(), true, GC.getDefineINT("EVENT_MESSAGE_TIME"), szBuffer, "AS2D_BOMBARD", MESSAGE_TYPE_INFO, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_GREEN"), pCity->getX_INLINE(), pCity->getY_INLINE());
			...
			if (pLoopPlot != NULL)
			{
				if (pLoopPlot->getNumUnits() > 0)
				{
					...
					if (pDefender != NULL)
					{
						if (pDefender->getDomainType() != DOMAIN_SEA && !isAnimal() && !pDefender->isAnimal())
						{
							pLoopPlot->setBattleFeature(true);
						}
