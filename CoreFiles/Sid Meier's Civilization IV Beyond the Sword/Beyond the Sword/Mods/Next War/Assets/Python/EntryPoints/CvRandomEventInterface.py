...
def canTriggerBillionsandBillions(argsList):
	...
	if pPlayer.isCivic(CvUtil.findInfoTypeNum(gc.getCivicInfo,gc.getNumCivicInfos(),'CIVIC_THEOCRACY')):
		return False
	if pPlayer.isCivic(CvUtil.findInfoTypeNum(gc.getCivicInfo,gc.getNumCivicInfos(),'CIVIC_FREE_SPEECH')):
		return True
...
def canApplyBillionsandBillions2(argsList):
	...
	iBuilding = CvUtil.findInfoTypeNum(gc.getBuildingInfo, gc.getNumBuildingInfos(), 'BUILDING_HOLLYWOOD')
	if player.getBuildingClassCount(gc.getBuildingInfo(iBuilding).getBuildingClassType()) == 0:
		return False
...
def canTriggerFiatMoney(argsList):
	...
	if pPlayer.getGold() > 100:
		return False
	if pPlayer.isCivic(CvUtil.findInfoTypeNum(gc.getCivicInfo,gc.getNumCivicInfos(),'CIVIC_DECENTRALIZATION')):
		return False
...
#   At least one civ on the board must know Gunpowder.
	bFoundValid = false
	iTech = CvUtil.findInfoTypeNum(gc.getTechInfo, gc.getNumTechInfos(), 'TECH_GUNPOWDER')
	for iPlayer in range(gc.getMAX_CIV_PLAYERS()):			
		loopPlayer = gc.getPlayer(iPlayer)
		if loopPlayer.isAlive():
			if gc.getTeam(loopPlayer.getTeam()).isHasTech(iTech):
...
def applyTheBuccaneers1(argsList):
	...
	iUnitType = CvUtil.findInfoTypeNum(gc.getUnitInfo, gc.getNumUnitInfos(), 'UNIT_FRENCH_MUSKETEER')
...
###     I've included the modern ships just to prevent anomalous triggering.
	...
	iSoL = CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_SHIP_OF_THE_LINE')
	iIronclad = CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_IRONCLAD')
	iStealth = CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_STEALTH_DESTROYER')
	iMissile = CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_MISSILE_CRUISER')
	iBoomer = CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_SUBMARINE')
	iSeawolf = CvUtil.findInfoTypeNum(gc.getUnitClassInfo, gc.getNumUnitClassInfos(), 'UNITCLASS_ATTACK_SUBMARINE')
...
def applyBlackbeard1(argsList):
	...
	iUnitType2 = CvUtil.findInfoTypeNum(gc.getUnitInfo, gc.getNumUnitInfos(), 'UNIT_FRIGATE')
	...
	###### NW_THE_CYLONS ######
#Event can only fire if using JKP1187's NOT JUST ANOTHER NEXTWAR MOD
...
def canTriggerNWSlipAndFall(argsList):
    	kTriggeredData = argsList[0]
	pPlayer = gc.getPlayer(kTriggeredData.ePlayer)
	iRoad = CvUtil.findInfoTypeNum(gc.getRouteInfo,gc.getNumRouteInfos(),'ROUTE_ROAD')
...
def canDoUnitNWSlipAndFall(argsList):
	eTrigger = argsList[0]
	ePlayer = argsList[1]
	iUnit = argsList[2]
	iCommando = CvUtil.findInfoTypeNum(gc.getPromotionInfo,gc.getNumPromotionInfos(),'PROMOTION_COMMANDO')
	...
	if pUnit.isHasPromotion(iCommando):
		return False


def applyNWSlipAndFall1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	...
	pUnit.setDamage(33, -1)


def getHelpNWSlipAndFall1(argsList):
	iEvent = argsList[0]
	kTriggeredData = argsList[1]
	...
	szHelp = localText.getText("TXT_KEY_EVENT_NW_SLIP_AND_FALL_1_HELP", ())	
