...
class CvDawnOfMan:
	...
	def interfaceScreen(self):
		...
		# Determine what the text will say
		bodyString = ""
		
		# Are we NOTE using the scenario file? If so, then show the ORIGINAL Dawn of Man text
		if (CyMap().plot(0,0).getScriptData() == "Scenario"):
			...
			bodyString = localText.getText("TXT_KEY_DAWN_OF_MAN_TEXT_MOD", (CyGameTextMgr().getTimeStr(gc.getGame().getGameTurn(), false), self.player.getCivilizationAdjectiveKey(), self.player.getNameKey()))
