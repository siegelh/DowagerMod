...
#		screen.hide( "CorporationAdvisorButton" )
	...
	def updateScreen(self):
		...
		g_EpidemicEffectTimer += 1
		if (g_EpidemicEffectTimer == 40):
			g_EpidemicEffectTimer = 0
			effect = gc.getInfoTypeForString('EFFECT_CITY_DISEASED')
			plots = []
			for unit, timeInfected in CvEventInterface.getEventManager().getEpidemic().getInfectedUnits():	
				# Check if we've fired an effect in this plot already
				shouldFire = True
				try:
					for plot in plots:
						if ((plot.getX() == unit.plot().getX() and plot.getY() == unit.plot().getY()) or unit.plot().getX() < 0 or unit.plot().getY() < 0 or not unit.plot().isVisibleToWatchingHuman()):
							shouldFire = False
				except:
					shouldFire = False
	...
	def updateSelectionButtons( self ):
		...
		print("Updating selection buttons")
			...
			if (CyInterface().getInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_SELECTION):
				...
				if ( pHeadSelectedUnit.getOwner() == gc.getGame().getActivePlayer() and g_pSelectedUnit != pHeadSelectedUnit ):
					...
					if (g_pSelectedUnit.plot().isCity()):
							if (g_pSelectedUnit.plot().getPlotCity().getOwner() == g_pSelectedUnit.getOwner() and g_pSelectedUnit.baseCombatStr() > 0):        
									# Culture ritual
									screen.appendMultiListButton("BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_CULTURE_SACRIFICE')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 444, 444, False)
									screen.show("BottomButtonContainer")
									iCount = iCount + 1
									
									# Happiness ritual
									screen.appendMultiListButton("BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_HAPPINESS_SACRIFICE')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 555, 555, False)
									screen.show("BottomButtonContainer")
									iCount = iCount + 1
					# END MESOAMERICA
	...
	def handleInput (self, inputClass):
		...
		if inputClass.getData1() == 444 and inputClass.getData2() == 444:
			
			# Send net message telling the game to sacrifice a unit
			
			unitID = CyInterface().getHeadSelectedUnit().getID()
			playerID = CyInterface().getHeadSelectedUnit().getOwner()
			
			CyMessageControl().sendModNetMessage(unitID, playerID, 0, -1, -1)
		
		elif inputClass.getData1() == 555 and inputClass.getData2() == 555:
			...
			CyMessageControl().sendModNetMessage(unitID, playerID, 1, -1, -1)
