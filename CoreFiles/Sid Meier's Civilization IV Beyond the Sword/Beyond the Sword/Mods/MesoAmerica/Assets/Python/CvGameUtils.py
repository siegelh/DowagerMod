...
class CvGameUtils:
	...
	def __init__(self): 
		self.mesoamericaEvents = CvEventInterface.getEventManager()
		pass
	...
	def AI_unitUpdate(self,argsList):
		...
		Mesoamerica = CvEventInterface.getEventManager()
		
		player = gc.getPlayer(pUnit.getOwner())
		if (not player.isHuman()):
			ships = Mesoamerica.getInvation().getShips()
			i = 0
			for ship, plot in ships:
				if (plot.getX() != ship.plot().getX() and plot.getY() != ship.plot().getY()):
					ship.getGroup().pushMission(MissionTypes.MISSION_MOVE_TO, plot.getX(), plot.getY(), 0, False, True, MissionAITypes.NO_MISSIONAI, plot, ship)
				else:
					ships.pop(i)
				i = i + 1
	...
	def doHolyCity(self):
		return True # Don't try to found holy city
	...
	def doCombat(self,argsList):
		...
		self.mesoamericaEvents.setAttackerPlot(pSelectionGroup.plot())
		return False
	...
	def getWidgetHelp(self, argsList):
		...
		if (iData1 == 444):
			helpText = localText.getText("TXT_KEY_CULTURE_SACRIFICE_UNIT_HELP", ())
		elif (iData1 == 555):
			helpText = localText.getText("TXT_KEY_HAPPINESS_SACRIFICE_UNIT_HELP", ())
