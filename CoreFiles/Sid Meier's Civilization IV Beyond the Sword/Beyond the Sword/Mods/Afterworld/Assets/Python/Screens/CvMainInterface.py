...
class CvMainInterface:
	...
	def interfaceScreen (self):
		...
		screen.setStyle( "EspionageAdvisorButton", "Button_HUDAdvisorForeign_Style" )
		...
		for i in range(MAX_CITIZEN_BUTTONS):
			...
			screen.setImageButton( szName, ArtFileMgr.getInterfaceArtInfo("INTERFACE_ANGRYCITIZEN_TEXTURE").getPath(), xResolution - 74 - (34 * i), yResolution - 248, 32, 32, WidgetTypes.WIDGET_ANGRY_CITIZEN, -1, -1 )
			screen.hide( szName )
		...
		for i in range( gc.getNumSpecialistInfos() ):
			if (gc.getSpecialistInfo(i).isVisible()):
				...
				screen.setButtonGFC( szName, u"", "", xResolution - 38, (yResolution - 282 - (34 * iCount)), 20, 20, WidgetTypes.WIDGET_CHANGE_SPECIALIST, i, 1, ButtonStyles.BUTTON_STYLE_CITY_PLUS )
				screen.hide( szName )
		...
		for i in range( gc.getNumSpecialistInfos() ):
			if (gc.getSpecialistInfo(i).isVisible()):
				...
				screen.setButtonGFC( szName, u"", "", xResolution - 38, (yResolution - 265 - (34 * iCount)), 20, 20, WidgetTypes.WIDGET_CHANGE_SPECIALIST, i, -1, ButtonStyles.BUTTON_STYLE_CITY_MINUS )
				screen.hide( szName )
		...
		for i in range( gc.getNumSpecialistInfos() ):
			...
			if (gc.getSpecialistInfo(i).isVisible()):
				...
				screen.setImageButton( szName, gc.getSpecialistInfo(i).getTexture(), xResolution - 74, (yResolution - 282 - (34 * i)), 32, 32, WidgetTypes.WIDGET_DISABLED_CITIZEN, i, -1 )
				screen.enable( szName, False )
				screen.hide( szName )

				for j in range(MAX_CITIZEN_BUTTONS):
					szName = "CitizenButton" + str((i * 100) + j)
					screen.addCheckBoxGFC( szName, gc.getSpecialistInfo(i).getTexture(), "", xResolution - 74 - (34 * j), (yResolution - 282 - (34 * i)), 32, 32, WidgetTypes.WIDGET_CITIZEN, i, j, ButtonStyles.BUTTON_STYLE_LABEL )
					screen.hide( szName )
		...
		pAfterworld.updateMinimapSection()
...
#			if pAfterworld.szMissionText != "No Objective":
#				screen.show( "MissionText" )
#			if pAfterworld.szMissionTitleText != "Title":
#				screen.show( "MissionTextTitle" )
#			#screen.show( "InterfaceHelpButton" )
			screen.show( "MainMenuButton" )
		else:
			screen.hide( "MissionText" )
			screen.hide( "MissionTextTitle" )
			screen.hide( "MissionTextPanelBackground" )
			screen.hide( "MissionTextPanel" )
			screen.hide( "BackButtonMissionText" )
			screen.hide( "ForwardButtonMissionText" )
			screen.hide( "BodyTankPic" )
			screen.hide( "BodyTankPicPanel" )
			screen.hide( "InterfaceHelpButton" )
			screen.hide( "MainMenuButton" )
	...
	def updatePlotListButtons( self ):
		...
		if ( CyInterface().shouldDisplayUnitModel() and CyEngine().isGlobeviewUp() == false and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL ):
			if ( CyInterface().isCitySelection() ):
				...
				for i in range( iOrders ):
					if ( bHandled == False ):
						...
						if (eOrderNodeType  == OrderTypes.ORDER_TRAIN ):
							screen.addUnitGraphicGFC( "InterfaceUnitModel", CyInterface().getOrderNodeData1(i), 190, yResolution - 138, 103, 132, WidgetTypes.WIDGET_HELP_SELECTED, 0, -1,  -20, 30, 1, False )
							bHandled = True
						elif ( eOrderNodeType == OrderTypes.ORDER_CONSTRUCT ):
							screen.addBuildingGraphicGFC( "InterfaceUnitModel", CyInterface().getOrderNodeData1(i), 190, yResolution - 138, 103, 132, WidgetTypes.WIDGET_HELP_SELECTED, 0, -1,  -20, 30, 0.8, False )
							bHandled = True
				...
				screen.addUnitGraphicGFC( "InterfaceUnitModel", CyInterface().getHeadSelectedUnit().getUnitType(), 190, yResolution - 138, 103, 132, WidgetTypes.WIDGET_UNIT_MODEL, CyInterface().getHeadSelectedUnit().getUnitType(), -1,  -20, 30, 1, False )
			else:
				screen.hide( "InterfaceUnitModel" )
		else:
			screen.hide( "InterfaceUnitModel" )
			...
			for i in range(pPlot.getNumUnits()):
				...
				if (pLoopUnit):
					...
					if ((iCount >= 0) and (iCount <  self.numPlotListButtons() * gc.getDefineINT("MAX_PLOT_LIST_ROWS"))):
						...
						#screen.addCheckBoxGFC( szString, gc.getUnitInfo(pLoopUnit.getUnitType()).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), 315 + (iCount * 34), yResolution - 169, 32, 32, WidgetTypes.WIDGET_PLOT_LIST, iCount + CyInterface().getPlotListColumn(), -1, ButtonStyles.BUTTON_STYLE_LABEL )
	...
	def updateFlag( self ):
		...
		if ( CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY ):
			...
			xResolution = screen.getXResolution()
			yResolution = screen.getYResolution()
			screen.addFlagWidgetGFC( "MissionText", xResolution - 288, yResolution - 138, 68, 250, gc.getGame().getActivePlayer(), WidgetTypes.WIDGET_FLAG, gc.getGame().getActivePlayer(), -1)
		...
		if (not CyEngine().isGlobeviewUp() and pHeadSelectedCity):
			...
			if (CyInterface().getInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_SELECTION):
				...
				if ( pHeadSelectedUnit.getOwner() == gc.getGame().getActivePlayer() and g_pSelectedUnit != pHeadSelectedUnit ):
					...
					if g_pSelectedUnit.canMove():
						#Turn Left
						screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_TURNLEFT')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 444, 444, False )
						screen.show( "BottomButtonContainer" )
						iCount = iCount + 1
						
						#Turn Right
						screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_TURNRIGHT')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 445, 445, False )
						screen.show( "BottomButtonContainer" )
						iCount = iCount + 1
					...
					for i in actions:
						...
						if gc.getActionInfo(i).getAutomateType() == AutomateTypes.AUTOMATE_EXPLORE or gc.getActionInfo(i).getCommandType() == CommandTypes.COMMAND_DELETE or gc.getActionInfo(i).getMissionType() == MissionTypes.MISSION_SENTRY or gc.getActionInfo(i).getMissionType() == MissionTypes.MISSION_MOVE_TO:
							continue
...
#						iCount = iCount + 1
					...
					# Don't do anything unless a unit is actually selected
					if (g_pSelectedUnit):
						
						# Define unit IDs
						iUnitIDRiest = CvUtil.findInfoTypeNum(gc.getUnitInfo,gc.getNumUnitInfos(),'UNIT_RIEST')
						iUnitIDJal = CvUtil.findInfoTypeNum(gc.getUnitInfo,gc.getNumUnitInfos(),'UNIT_JAL')
						iUnitIDAtticus = CvUtil.findInfoTypeNum(gc.getUnitInfo,gc.getNumUnitInfos(),'UNIT_ATTICUS')
						iUnitIDSever = CvUtil.findInfoTypeNum(gc.getUnitInfo,gc.getNumUnitInfos(),'UNIT_SEVER')
						iUnitIDRagah = CvUtil.findInfoTypeNum(gc.getUnitInfo,gc.getNumUnitInfos(),'UNIT_RAGAH')
						iUnitIDSeverInvisible = CvUtil.findInfoTypeNum(gc.getUnitInfo,gc.getNumUnitInfos(),'UNIT_SEVER_INVISIBLE')
						
						pAfterworld.pPushedButtonUnit = g_pSelectedUnit
						
						# If unit can be promoted, set them to be promotion ready
						if pAfterworld.pPushedButtonUnit.getExperience() >= pAfterworld.pPushedButtonUnit.experienceNeeded():
							pAfterworld.pPushedButtonUnit.setPromotionReady(1)

						# Control button
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_CONTROL1')):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if (g_pSelectedUnitData.getControlTimer() == 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_CONTROL1')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 660, 660, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1
	
						# Projection button
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_WARP1')):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if (g_pSelectedUnitData.getProjectTimer() == 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_WARP1')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 661, 661, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1
						...
						# Barrier button
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_BARRIER1')):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if (g_pSelectedUnitData.getBarrierTimer() == 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_BARRIER1')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 663, 663, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1
	
						# Enhance Moves button	
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_ENH_AC1')):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if (g_pSelectedUnitData.getEnhAccTimer() == 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_ENH_AC1')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 664, 664, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1
	
						# Shadow Walk button
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_SHADOWWALK1')):
								
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if (g_pSelectedUnitData.getShadowTimer() == 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_SHADOWWALK1')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 665, 665, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1
	
						# MDA button
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_MDA1')): 
							#print "ID"
							#print pHeadSelectedUnit.getID()
							#print("pAfterworld.iNumUnitDatas")
							#print(pAfterworld.iNumUnitDatas)
							#print("pAfterworld.apUnitDatas")
							#print(pAfterworld.apUnitDatas)
							
							for i in range(pAfterworld.iNumUnitDatas):
								pUnitData = pAfterworld.getUnitDataByIndex(i)
								#print(pUnitData.getOutputData())
							
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							
							if (g_pSelectedUnitData.getMDATimer() == 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_MDA1')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 666, 666, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1
	
						# Silence button
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_SILENCE')):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if (g_pSelectedUnit.getUnitType() == iUnitIDSeverInvisible):
								if (g_pSelectedUnitData.getSilenceTimer() == 0):
									screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_SILENCE')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 669, 669, False )
									screen.show( "BottomButtonContainer" )
									iCount = iCount + 1
	
						# MAUL button
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_MAULER1')):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if (g_pSelectedUnitData.getMAULerTimer() == 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_MAULER1')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 670, 670, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1
	

						#Plant Detonator
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_DETONATOR1')):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if (g_pSelectedUnitData.getDetonatorTimer() == 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_DETONATOR1')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 672, 672, False )
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_DETONATOR2')):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if (g_pSelectedUnitData.getDetonatorTimer() == 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_DETONATOR2')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 672, 672, False )								
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1
						...
						#Spawn Sentinel 2
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_SPAWN2')):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if (g_pSelectedUnitData.getSpawn2Timer() == 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_SPAWN2')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 684, 684, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1

						#Spawn Sentinel 3
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_SPAWN3')):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if (g_pSelectedUnitData.getSpawn3Timer() == 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_SPAWN3')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 685, 685, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1

						#Repair Complete
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_RESTORE3')):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if (g_pSelectedUnitData.getRepairTimer() == 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_RESTORE3')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 680, 680, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1
						
						#Deception Beacon
						g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
						if g_pSelectedUnitData:
							if (g_pSelectedUnitData.getNumDeceptionBeacon() > 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_DECEP')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 781, 781, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1

						#Detonator III
						if g_pSelectedUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_DETONATOR3')):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							print g_pSelectedUnitData.getGrenadeTimer()
							print "Grenade Timer"
							if (g_pSelectedUnitData.getGrenadeTimer() == 0):
								screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_DETONATOR3')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 683, 683, False )
								screen.show( "BottomButtonContainer" )
								iCount = iCount + 1

						#Repair Mechanism Button
						if (g_pSelectedUnit.getUnitType() == iUnitIDRiest or g_pSelectedUnit.getUnitType() == iUnitIDJal or g_pSelectedUnit.getUnitType() == iUnitIDSever or g_pSelectedUnit.getUnitType() == iUnitIDSeverInvisible or g_pSelectedUnit.getUnitType() == iUnitIDRagah or g_pSelectedUnit.getUnitType() == iUnitIDAtticus):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if g_pSelectedUnitData:
								
								print("   pHeadSelectedUnit.getID(): %d" %(pHeadSelectedUnit.getID()))
								print("   g_pSelectedUnitData.iNumRepairMechanism: %d" %(g_pSelectedUnitData.iNumRepairMechanism))
								if (g_pSelectedUnitData.getNumRepairMechanism() > 0):
									screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_MECHS')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 681, 681, False )
									screen.show( "BottomButtonContainer" )
									iCount = iCount + 1

						#Acceleration Mechanism Button
						if (g_pSelectedUnit.getUnitType() == iUnitIDRiest or pHeadSelectedUnit.getUnitType() == iUnitIDJal or pHeadSelectedUnit.getUnitType() == iUnitIDSever or g_pSelectedUnit.getUnitType() == iUnitIDSeverInvisible or pHeadSelectedUnit.getUnitType() == iUnitIDRagah or pHeadSelectedUnit.getUnitType() == iUnitIDAtticus):
							g_pSelectedUnitData = pAfterworld.getUnitDataByID(pHeadSelectedUnit.getID())
							if g_pSelectedUnitData:
								if (g_pSelectedUnitData.getNumAccelerationMechanism() > 0):
									screen.appendMultiListButton( "BottomButtonContainer", gc.getPromotionInfo(gc.getInfoTypeForString('PROMOTION_MECHS')).getButton(), 0, WidgetTypes.WIDGET_GENERAL, 682, 682, False )
									screen.show( "BottomButtonContainer" )
									iCount = iCount + 1
			...
			if (pHeadSelectedCity and CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_SHOW):
				...
				for i in range( gc.getNumSpecialistInfos() ):
					...
					for j in range( iCount ):
						...
						screen.show( szName )
						szName = "CitizenButtonHighlight" + str((i * 100) + j)
						screen.addDDSGFC( szName, ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), xResolution - 74 - (34 * j), (yResolution - 282 - (34 * i)), 32, 32, WidgetTypes.WIDGET_CITIZEN, i, j )
	...
	def updateCityScreen( self ):
		...
		screen.addPanel( "BonusPane0", u"", u"", True, False, xResolution - 244, 94, 57, yResolution - 588, PanelStyles.PANEL_STYLE_CITY_COLUMNL )
		screen.hide( "BonusPane0" )
		screen.addScrollPanel( "BonusBack0", u"", xResolution - 242, 94, 157, yResolution - 604, PanelStyles.PANEL_STYLE_EXTERNAL )
		screen.hide( "BonusBack0" )

		screen.addPanel( "BonusPane1", u"", u"", True, False, xResolution - 187, 94, 68, yResolution - 588, PanelStyles.PANEL_STYLE_CITY_COLUMNC )
		screen.hide( "BonusPane1" )
		screen.addScrollPanel( "BonusBack1", u"", xResolution - 191, 94, 184, yResolution - 604, PanelStyles.PANEL_STYLE_EXTERNAL )
		screen.hide( "BonusBack1" )

		screen.addPanel( "BonusPane2", u"", u"", True, False, xResolution - 119, 94, 107, yResolution - 588, PanelStyles.PANEL_STYLE_CITY_COLUMNR )
		screen.hide( "BonusPane2" )
		screen.addScrollPanel( "BonusBack2", u"", xResolution - 125, 94, 205, yResolution - 604, PanelStyles.PANEL_STYLE_EXTERNAL )
			...
			if ( pHeadSelectedCity ):
				...
				for i in range(gc.getDefineINT("MAX_TRADE_ROUTES")):
					...
					if (pLoopCity and pLoopCity.getOwner() >= 0):
						...
						screen.setTableText( "TradeRouteTable", 0, iNumTradeRoutes, "<font=1>" + szLeftBuffer + "</font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
	...
	def updateScoreStrings( self ):
		...
		if (CyInterface().isScoresVisible() and not CyInterface().isCityScreenUp() and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY and CyEngine().isGlobeviewUp() == false):
...
#		screen.hide( "GlobeToggle" )
	...
	def updateMissionText( self ):
		...
		if ( CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY ):
			...
			AfterworldMessage = pAfterworld.getCurrentAfterworldMessage()

			if AfterworldMessage:
				
				screen.addPanel( "MissionTextPanelBackground", u"", u"", False, False, xResolution - 284, yResolution - 283, 295, 138, PanelStyles.PANEL_STYLE_MAIN )
				screen.addPanel( "MissionTextPanel", u"", u"", False, False, xResolution - 280, yResolution - 280, 295, 138, PanelStyles.PANEL_STYLE_STANDARD )
				screen.setStyle( "MissionTextPanel", "Panel_Game_HudStat_Style" )
	
				missionTitleText = AfterworldMessage[0]
				missionText = AfterworldMessage[1]
				
				#bodyTankPic = gc.getUnitInfo(gc.getInfoTypeForString("UNIT_JAL")).getButton()
				
				bodyTankPic = pAfterworld.getButton()
				
				if bodyTankPic != "None":
					screen.addPanel( "BodyTankPicPanel", u"", u"", False, False, xResolution - 344, yResolution - 282, 64, 64, PanelStyles.PANEL_STYLE_MAIN )
					screen.setImageButtonAt( "BodyTankPic", "BodyTankPicPanel", bodyTankPic, 0,0,64,64,WidgetTypes.WIDGET_GENERAL,-1,-1 )
					screen.moveToFront( "BodyTankPic" )
				...
				if missionTitleText:
					uMissionTitleText = u"%s" %missionTitleText
					screen.addMultilineText( "MissionTextTitle", uMissionTitleText, xResolution - 280, yResolution - 278, 270, 25, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
				
				if missionText:
					uMissionText = u"%s" %missionText
					screen.addMultilineText( "MissionText", uMissionText, xResolution - 280, yResolution - 253, 270, 100, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
			
				pAfterworld.bNewAfterworldMessage = False
			
				if pAfterworld.iCurrentAfterworldMessage != 0:
					#screen.setImageButton( "BackButtonMissionText", ",Art/Interface/Buttons/Brown_empty.dds,Art/Interface/Buttons/Afterworld_Atlas.dds,7,13", xResolution - 282, yResolution - 135, 32, 32, WidgetTypes.WIDGET_GENERAL, 951, -1 )
					screen.setImageButton( "BackButtonMissionText", "Art/Interface/Buttons/LeftArrow.dds", xResolution - 282, yResolution - 135, 32, 32, WidgetTypes.WIDGET_GENERAL, 951, -1 )
					screen.moveToFront( "BackButtonMissionText" )
				else:
					screen.hide("BackButtonMissionText")
	
				if pAfterworld.iCurrentAfterworldMessage != len(pAfterworld.AfterworldMessages)-1:
					#screen.setImageButton( "ForwardButtonMissionText", ",Art/Interface/Buttons/Brown_empty.dds,Art/Interface/Buttons/Afterworld_Atlas.dds,8,13", xResolution - 248, yResolution - 135, 32, 32, WidgetTypes.WIDGET_GENERAL, 952, -1 )
					screen.setImageButton( "ForwardButtonMissionText", "Art/Interface/Buttons/RightArrow.dds", xResolution - 248, yResolution - 135, 32, 32, WidgetTypes.WIDGET_GENERAL, 952, -1 );
					screen.moveToFront( "ForwardButtonMissionText" )
				else:
					pAfterworld.fAfterworldTimer = 0.0
					screen.hide("ForwardButtonMissionText")

	def update(self, fDelta):
		if pAfterworld.bNewAfterworldMessage:
			if pAfterworld.fAfterworldTimer != -1.0:
				AfterworldMessage = pAfterworld.getNextAfterworldMessage()
			else:
				AfterworldMessage = pAfterworld.getCurrentAfterworldMessage()
			
			if AfterworldMessage:
				pAfterworld.fAfterworldTimer = pAfterworld.getCurrentAfterworldMessage()[2]
				self.updateMissionText()
		
		if pAfterworld.fAfterworldTimer > 0:
			pAfterworld.fAfterworldTimer -= fDelta
		
		elif pAfterworld.fAfterworldTimer < 0 or pAfterworld.fAfterworldTimer != -1.0:
			pAfterworld.bNewAfterworldMessage = True
			return
				
		return

	def doRepairMechanism(self):
		pUnit = g_pSelectedUnit
		pUnit.changeDamage(-100, 0)
		
	def doAccelerationMechanism(self):
		pUnit = g_pSelectedUnit
		pUnit.setMoves(0)

	def doShadowWalk(self):
		
		# Update unit data just before we create a new unit
		CvEventInterface.getEventManager().updateUnitData()
		
		iID = g_pSelectedUnit.getID()
		pUnitData = pAfterworld.getUnitDataByID(iID)
		...
		print "This is the ID before"
		print iID
		pUnitData.setShadowTimer(8)
		pUnitData.setInvisibilityTurns(4)
		print(pUnitData.iNumRepairMechanism)
		print("pUnitData.iNumRepairMechanism")
		
		plotX = g_pSelectedUnit.getX()
		plotY = g_pSelectedUnit.getY()
		...
		print "Just killed him and his ID is"
		print iID
		pPlayer.initUnit(iShadow, plotX, plotY, UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION) #And now you don't... - Create invisible Sever and carryover  Sever's info#
		...
		for iUnitLoop in range(pPlot.getNumUnits()):
			...
			if (pUnitLoop.getUnitType() == iShadow):
		...
		for iUnitDataLoop in range(pAfterworld.getNumUnitDatas()):
			pData = pAfterworld.getUnitDataByIndex(iUnitDataLoop)
			print("Unit Data ID")
			print(pData.getID())
		
#		iNewID = CyInterface().getHeadSelectedUnit().getID()
#		iInv = CyInterface().getHeadSelectedUnit().getID()
#		pInvData = pAfterworld.getUnitDataByID(iInv)
#		print "Just killed him and his ID is"
#		print iNewID		
		
#		pInvData.setGrenadeTimer(iGrenades)
#		pInvData.setNumRepairMechanism(iRepairMechs)
#		pInvData.setNumAccelerationMechanism(iAccMechs)
#		pInvData.setNumDeceptionBeacon(iDBeacons)
#		pInvData.setSilenceTimer(iSilence)
#		pInvData.setExperience(iExperience)
#		pInvData.setNumPromotions(iPromotions)
#		pInvData.setDetonatorTimer(iDetonator)
#		pInvData.setInvisibilityTurns(2)

	def doMDADevastation(self, iX, iY):
		...
		iFeatureType = plot.getFeatureType()
		if(iFeatureType == iW or iFeatureType == iT1 or iFeatureType == iT2 or iFeatureType == iT3):
			plot.setFeatureType(iGround,1)
		if plot.isUnit():
			iUnits = gc.getMap().plot(iX, iY).getNumUnits()
			for i in range(iUnits):
				pUnit = plot.getUnit(i)
				if pUnit.getOwner() != 0:
					pUnit.changeDamage(100, 0)

	def doMDA(self, iX, iY):
		...
		for i in range(3):
			self.doMDADevastation(plotX - i, plotY )
			self.doMDADevastation (plotX - i, plotY - i)
			self.doMDADevastation (plotX, plotY - i)
			self.doMDADevastation (plotX + i, plotY - i)
			self.doMDADevastation (plotX + i, plotY)
			self.doMDADevastation (plotX + i, plotY + i)
			self.doMDADevastation (plotX, plotY + i)
	...
	def handleInput (self, inputClass):
		...
		if g_pSelectedUnit == None:
			return
		if g_pSelectedUnit.isNone():
			return
		
		iID = g_pSelectedUnit.getID()
		pAfterworld.pPushedButtonUnit = CyInterface().getHeadSelectedUnit()
		pUnitData = pAfterworld.getUnitDataByID(iID)
		
		iX = g_pSelectedUnit.getX()
		iY = g_pSelectedUnit.getY()
		
		pGoodbye = CyMap().plot(iX, iY)
		if not g_pSelectedUnit.isCombat():
			
			
			if inputClass.getData1() == 665: #Shadow 
				
				self.doShadowWalk()
	
			elif inputClass.getData1() == 666:  #MDA
				pUnitData.setMDATimer(10)
				CyEngine().triggerEffect(3, pGoodbye.getPoint())
				
				effectType1 = gc.getInfoTypeForString('EFFECT_ICBM_NUCLEAR_EXPLOSION')
				effectType2 = gc.getInfoTypeForString('EFFECT_EXPLOSION_SHOCKWAVE')
	
				CyEngine().triggerEffect(effectType1, pGoodbye.getPoint())
				CyEngine().triggerEffect(effectType2, pGoodbye.getPoint())
				self.doMDA(iX, iY)
	
			elif inputClass.getData1() == 660:  #Control
				pAfterworld.pPushButton = 1
				
				CyInterface().setInterfaceMode(InterfaceModeTypes.INTERFACEMODE_PYTHON_PICK_PLOT)
	
			elif inputClass.getData1() == 661: #Project
				pAfterworld.pPushButton = 2
				
				CyInterface().setInterfaceMode(InterfaceModeTypes.INTERFACEMODE_PYTHON_PICK_PLOT)
	
			elif inputClass.getData1() == 662:	#Inhibitor
				pAfterworld.pPushButton = 3
				
				CyInterface().setInterfaceMode(InterfaceModeTypes.INTERFACEMODE_PYTHON_PICK_PLOT)
	
			elif inputClass.getData1() == 663:	#Barrier
				pAfterworld.pPushButton = 4
				
				CyInterface().setInterfaceMode(InterfaceModeTypes.INTERFACEMODE_PYTHON_PICK_PLOT)
	
			elif inputClass.getData1() == 664:	#Enhance Acceleration
				pAfterworld.pPushButton = 5
				
				CyInterface().setInterfaceMode(InterfaceModeTypes.INTERFACEMODE_PYTHON_PICK_PLOT)
	
			elif inputClass.getData1() == 669:	#Silence
				pAfterworld.pPushButton = 7
				
				CyInterface().setInterfaceMode(InterfaceModeTypes.INTERFACEMODE_PYTHON_PICK_PLOT)
	
			elif inputClass.getData1() == 670:	#MAULer
				pAfterworld.pPushButton = 8
				
				CyInterface().setInterfaceMode(InterfaceModeTypes.INTERFACEMODE_PYTHON_PICK_PLOT)
	
			elif inputClass.getData1() == 672:	#Detonator
				pAfterworld.pPushButton = 9
			...
			elif inputClass.getData1() == 684:	#Spawn Sentinel 2
				pAfterworld.pPushButton = 14
				CyInterface().setInterfaceMode(InterfaceModeTypes.INTERFACEMODE_PYTHON_PICK_PLOT)
	
			elif inputClass.getData1() == 685:	#Spawn Sentinel 3
				pAfterworld.pPushButton = 15
				CyInterface().setInterfaceMode(InterfaceModeTypes.INTERFACEMODE_PYTHON_PICK_PLOT)
	
			elif inputClass.getData1() == 680:	#Repair Complete 1
				pAfterworld.pPushButton = 11
				
				CyInterface().setInterfaceMode(InterfaceModeTypes.INTERFACEMODE_PYTHON_PICK_PLOT)
				
			elif inputClass.getData1() == 681:	#Isogen Repair Mechanism
				pUnitData.changeNumRepairMechanism(-1)
				self.doRepairMechanism()
				
			elif inputClass.getData1() == 682:	#Isogen Acceleration Mechanism
				pUnitData.changeNumAccelerationMechanism(-1)
				self.doAccelerationMechanism()
				
			elif inputClass.getData1() == 444: #Turn Left
				g_pSelectedUnit.changeMoves(15)
				g_pSelectedUnit.rotateFacingDirectionCounterClockwise()
	
			elif inputClass.getData1() == 445: #Turn Right
				g_pSelectedUnit.changeMoves(15)
				g_pSelectedUnit.rotateFacingDirectionClockwise()
				
			elif inputClass.getData1() == 683: #Detonator 3
				pAfterworld.pPushButton = 12
				CyInterface().setInterfaceMode(InterfaceModeTypes.INTERFACEMODE_PYTHON_PICK_PLOT)
				
			elif inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED and inputClass.getData1() == 951:
	#			if (inputClass.getFlags() == MouseFlags.MOUSE_LBUTTONDOWN or inputClass.getFlags() == MouseFlags.MOUSE_RBUTTONDOWN):
	#				print "Next Message"
				pAfterworld.getPreviousAfterworldMessage()
				self.updateMissionText()
				pAfterworld.fAfterworldTimer = -1
			
			elif inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED and inputClass.getData1() == 952:
	#			if (inputClass.getFlags() & MouseFlags.MOUSE_LBUTTONDOWN or inputClass.getFlags() & MouseFlags.MOUSE_RBUTTONDOWN):
				pAfterworld.getNextAfterworldMessage()
				self.updateMissionText()
				pAfterworld.fAfterworldTimer = -1
