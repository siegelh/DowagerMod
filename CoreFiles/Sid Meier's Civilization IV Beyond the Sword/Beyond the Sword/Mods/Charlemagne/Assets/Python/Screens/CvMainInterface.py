...
class CvMainInterface:
								...
								for iReligionLoop in range(gc.getNumReligionInfos()):
									...
									if (pCity.isHasReligion(iReligionLoop)):
										...
										if (pPlayer.getStateReligion() != iReligionLoop):
											...
											screen.appendMultiListButton( "BottomButtonContainer", ArtFileMgr.getInterfaceArtInfo("INTERFACE_CHARLEMAGNE_PERSICUTION").getPath(), 0, WidgetTypes.WIDGET_GENERAL, 666, 666, False )
											screen.show( "BottomButtonContainer" )
											iCount = iCount + 1
	...
	def updateGameDataStrings( self ):
		...
		screen.hide( "TimeText" )
		screen.hide( "ResearchBar" )
		screen.hide( "CharlemagneText" )	# Charlemagne
		screen.hide( "CharlemagneBackground" )
		...
		if ( CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY  and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_ADVANCED_START):
			...
			if (gc.getPlayer(ePlayer).isAlive()):
				...
				if (not CyInterface().isCityScreenUp()):
					...
					pCharlemagne.initValues()
					
					szHRE = localText.getText("TXT_KEY_NONE", ())
					szExcommunicated = localText.getText("TXT_KEY_NONE", ())
					iAdditional = 0
					
					# Determine favor list
					aaiFavorList = []
					for iPlayerLoop in range(5):
						pPlayer = gc.getPlayer(iPlayerLoop)
						szText = u"<color=%d,%d,%d,%d>%d: %s</color>\n" %(pPlayer.getPlayerTextColorR(), pPlayer.getPlayerTextColorG(), pPlayer.getPlayerTextColorB(), pPlayer.getPlayerTextColorA(), pCharlemagne.getPlayerTotalFavorWithRome(iPlayerLoop), pPlayer.getName())
						aaiFavorList.append([pCharlemagne.getPlayerTotalFavorWithRome(iPlayerLoop), szText])
						
						# HRE
						if (pCharlemagne.isHolyRomanEmperor(iPlayerLoop)):
							if szHRE == localText.getText("TXT_KEY_NONE", ()):
								szHRE = ""
							
							szHRE = u"<color=%d,%d,%d,%d>%s</color>\n" %(pPlayer.getPlayerTextColorR(), pPlayer.getPlayerTextColorG(), pPlayer.getPlayerTextColorB(), pPlayer.getPlayerTextColorA(), pPlayer.getName())
							iAdditional += 20
						
						# Excommunicated
						if (pCharlemagne.getPlayerTotalFavorWithRome(iPlayerLoop) <= pCharlemagne.m_iExcommunicatedValue):
							if szExcommunicated == "None":
								szExcommunicated == ""
							
							szExcommunicated += u"<color=%d,%d,%d,%d>%s</color>\n" %(pPlayer.getPlayerTextColorR(), pPlayer.getPlayerTextColorG(), pPlayer.getPlayerTextColorB(), pPlayer.getPlayerTextColorA(), pPlayer.getName())
							iAdditional += 20
					
					aaiFavorList.sort()
					aaiFavorList.reverse()
					
					# Now generate string
					szText = "%s:\n" %(localText.getText("TXT_KEY_CHARL_FAVOR_WITH_ROME", ()))
					for iPlayerLoop in range(5):
						pPlayer = gc.getPlayer(iPlayerLoop)
						iFavor = aaiFavorList[iPlayerLoop][0]
						szTextLoop = aaiFavorList[iPlayerLoop][1]
						szText += szTextLoop
					
					if (szHRE != localText.getText("TXT_KEY_NONE", ())):
						iAdditional += 20
						szHRE = szHRE.strip("None")
						szText += "\n%s:\n%s" % (localText.getText("TXT_KEY_CHARL_HOLY_ROMAN_EMPEROR", ()), szHRE)
					
					if (szExcommunicated != localText.getText("TXT_KEY_NONE", ())):
						iAdditional += 20
						szExcommunicated = szExcommunicated.strip("None")
						szText += "\n%s:\n%s" % (localText.getText("TXT_KEY_CHARL_EXCOMMUNICATED", ()), szExcommunicated)
					
					szName = "CharlemagneText"
					iX = screen.getXResolution() - 170 - 20
					iY = 125
					iWidth = 160
					iHeight = 165 + iAdditional
					
					screen.addPanel( "CharlemagneBackground", u"", u"", True, False, iX-5, iY-10, iWidth+10, iHeight+10, PanelStyles.PANEL_STYLE_HUD_HELP )
					screen.show("CharlemagneBackground")
					
					screen.addMultilineText(szName, szText, iX, iY, iWidth, iHeight, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					screen.show(szName)
