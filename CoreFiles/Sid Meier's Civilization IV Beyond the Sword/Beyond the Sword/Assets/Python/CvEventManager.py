...
## Terraform End (avain)
	...
	def onChat(self, argsList):
		...
		# DowagerMod chatter spike (Pre-flight B): inspect what onChat actually
		# receives and try every render/suppress strategy. Remove after the
		# Leader Chatter feature ships its real onChat handler.
		try:
			self._chatterSpikeLog(argsList)
			self._chatterSpikeMaybeRender(chatMessage)
		except:
			# Spike must NEVER raise into the engine. Swallow everything.
			pass
		# Last-ditch suppression test: return True / "True" / 1 to see if BtS
		# treats any of these as "do not display the raw chat in the panel".
		# We toggle which sentinel based on a special prefix in the message
		# so we can compare runs.
		if chatMessage.startswith("!chattersuppress-true"):
			return True
		if chatMessage.startswith("!chattersuppress-1"):
			return 1
		if chatMessage.startswith("!chattersuppress-str"):
			return "True"
		if chatMessage.startswith("!chattersuppress-none"):
			return None

	def _chatterSpikeLog(self, argsList):
		"""Pre-flight B: log every onChat call with full arg metadata."""
		import os, time
		try:
			docs = os.path.expanduser("~\\Documents\\My Games\\Beyond the Sword\\Logs")
			if not os.path.isdir(docs):
				return
			path = os.path.join(docs, "ChatterSpike.log")
			f = open(path, "a")
			try:
				ts = time.strftime("%Y-%m-%d %H:%M:%S")
				f.write("[%s] onChat fired, len(argsList)=%d\n" % (ts, len(argsList)))
	...
	def _chatterSpikeMaybeRender(self, chatMessage):
		"""Pre-flight B: if the magic prefix is seen, render via addMessage
		with several variants so we can see which looks best in-game.
		Triggered by typing in chat:  !chatterprobe
		"""
		if not chatMessage.startswith("!chatterprobe"):
			return
		import os, time
		try:
			docs = os.path.expanduser("~\\Documents\\My Games\\Beyond the Sword\\Logs")
			...
			f = open(path, "a")
			try:
				f.write("[%s] !chatterprobe seen, attempting addMessage variants\n"
			...
			pass
		try:
			iLocal = gc.getGame().getActivePlayer()
			iSpeaker = -1
			# Find any AI player to use as the synthetic "speaker" so we can
			# observe portrait/color attribution.
			for i in range(gc.getMAX_CIV_PLAYERS()):
				p = gc.getPlayer(i)
				if p.isAlive() and not p.isHuman() and p.getLeaderType() != -1:
					iSpeaker = i
					break
			if iSpeaker < 0:
				iSpeaker = iLocal
			leaderName = gc.getPlayer(iSpeaker).getName()
			color = gc.getPlayer(iSpeaker).getPlayerTextColorType()

			# Variant 1: plain MAJOR_EVENT, prefixed line, speaker color, no icon.
			line1 = u"[ChatterSpike v1 plain] %s -> you: An imagined remark, witty and arch." % leaderName
			CyInterface().addMessage(
				iLocal, False, 8, line1, "",
				InterfaceMessageTypes.MESSAGE_TYPE_MAJOR_EVENT,
				None, color, -1, -1, True, True
			)

			# Variant 2: with leader portrait button if available.
			leaderType = gc.getPlayer(iSpeaker).getLeaderType()
			leaderInfo = gc.getLeaderHeadInfo(leaderType)
			leaderArt = leaderInfo.getButton() if leaderInfo else None
			line2 = u"[ChatterSpike v2 portrait] %s: A line testing leader-portrait icon." % leaderName
			CyInterface().addMessage(
				iLocal, False, 8, line2, "",
				InterfaceMessageTypes.MESSAGE_TYPE_MAJOR_EVENT,
				leaderArt, color, -1, -1, True, True
			)

			# Variant 3: minor event, no popup (just slides into log).
			line3 = u"[ChatterSpike v3 minor] %s remarks something subtler." % leaderName
			CyInterface().addMessage(
				iLocal, False, 8, line3, "",
				InterfaceMessageTypes.MESSAGE_TYPE_MINOR_EVENT,
				leaderArt, color, -1, -1, True, True
			)

			# Variant 4: addImmediateMessage (chat-area sibling).
			CyInterface().addImmediateMessage(
				u"[ChatterSpike v4 immediate] %s says: a chat-area test." % leaderName, ""
			)
		except:
			pass
