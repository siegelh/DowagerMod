...
# Petromod
		if iBuildingType == CvUtil.findInfoTypeNum(gc.getBuildingInfo, gc.getNumBuildingInfos(), "BUILDING_REFINERY"):
			icorp = CvUtil.findInfoTypeNum(gc.getCorporationInfo, gc.getNumCorporationInfos(), "CORPORATION_3")
...
# Petromod
	...
	def onCorporationRemove(self, argsList):
		...
		if (not self.__LOG_RELIGIONSPREAD):
			return
		CvUtil.pyPrint('%s has been removed from Player %d Civilization %s city of %s'
			%(gc.CorporationInfo(iCorporation).getDescription(), iOwner, player.getCivilizationName(), pRemoveCity.getName()))
