...
- Great Person landmarks are a generic, XML-backed improvement layer: `CvImprovementInfo` (in `CvInfos.{h,cpp}`) carries neutral-default landmark fields (logical group, spacing, city-adjacency, coastal, state-religion gating, and a `LandmarkTypes` behavior selector) parsed from `CIV4TerrainSchema.xml`. Placement legality is centralized in `CvPlot::canBuildLandmark` (shared by human and AI through `CvPlot::canBuild`); source-tile adjacency and the non-stacking Naval Foundry radius-two water aura are computed in `CvPlot::calculateImprovementYieldChange`/`calculateYield`; Research Campus injects Research into the working city before modifiers via `CvCity::calculateImprovementCityCommerceFromTraitsAndCivics`; help is generated in `CvGameTextMgr`; and deterministic AI placement lives in `CvUnitAI::AI_buildGreatPersonLandmark`/`AI_scoreLandmarkBuild`, integrated into the Engineer/Scientist/Merchant/Prophet moves and the Venetian Merchant Prince decision. Improvement/build/art/text records are appended only, and the roster-safety art check accepts reused stock FPK art via `tools/baselines/packed_stock_art.json`.
- Diplomatic espionage is a DLL-backed, XML-configured extension. `CvEspionageMissionInfo` identifies Stage Diplomatic Incident, Establish Backchannels, and Fabricate Casus Belli; `CvPlayer` centralizes physical spy/plot validation, target eligibility, costs, synchronized effects, and Casus Belli odds. The native two-phase popup carries selected third-party player IDs through synchronized `MISSION_ESPIONAGE` extra data and displays the same shared Casus Belli chance used during execution. Stage Diplomatic Incident applies a persistent one-way `AI_changeAttitudeExtra` penalty, Establish Backchannels adds one save-backed and naturally decaying `MEMORY_GIVE_HELP` count, and successful Casus Belli fabrication creates `WARPLAN_PREPARING_LIMITED` only after synchronized RNG. `CvPlayerAI` evaluates all three deterministically without asynchronous randomness.
...
8. Two-phase espionage missions remain synchronized in the DLL: the first popup selects a mission, `BUTTONPOPUP_DOESPIONAGE_TARGET` selects mission-specific extra data, and the resulting `MISSION_ESPIONAGE` command performs validation and mutation. The staged diplomatic incident uses a framed player ID as that extra data and persists its effect through the existing serialized attitude-extra matrix.
