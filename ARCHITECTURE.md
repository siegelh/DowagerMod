...
- Staged diplomatic incidents are a DLL-backed, XML-configured espionage extension. `CvEspionageMissionInfo` carries `bStagesDiplomaticIncident` and `iDiplomaticAttitudeChange`; the native two-phase espionage popup sends the selected framed player through synchronized `MISSION_ESPIONAGE` extra data; and `CvPlayer` validates the spy, plot, AI-controlled territory owner, and framed civilization before applying a persistent one-way `AI_changeAttitudeExtra` modifier. `CvPlayerAI` uses the same eligibility contract and deterministic visible-attitude-threshold valuation.
